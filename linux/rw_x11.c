...
#define STD_EVENT_MASK \
	(StructureNotifyMask | KeyPressMask | KeyReleaseMask | ExposureMask | PointerMotionMask | ButtonPressMask | ButtonReleaseMask)
...
void ResetFrameBuffer (void)
{
	...
	x_framebuffer[0] = XCreateImage (x_disp, x_vis, x_visinfo->depth, ZPixmap, 0, malloc (mem), vid.width, vid.height, 32, 0);
...
void ResetSharedFrameBuffers (void)
{
	...
	for (frm = 0; frm < 2; frm++)
	{
		...
		ri.Con_Printf (PRINT_ALL, "MITSHM shared memory (id=%d, addr=0x%lx)\n", x_shminfo[frm].shmid, (long) x_shminfo[frm].shmaddr);
...
// If not specified, use default visual
	...
	if (num_visuals > 1)
	{
		...
		for (i = 0; i < num_visuals; i++) printf ("	-visualid %d\n", (int) (x_visinfo[i].visualid));
	}
	...
	if (XShmQueryExtension (x_disp))
	{
		char *displayname;
		doShm		= true;
		displayname = (const char *) getenv ("DISPLAY");
		if (displayname)
		{
			char *d = displayname;
			while (*d && (*d != ':')) d++;
...
// if the window changes dimension, skip this frame
	...
	if (doShm)
	{
		...
		if (!XShmPutImage (x_disp, x_win, x_gc, x_framebuffer[current_framebuffer], 0, 0, 0, 0, vid.width, vid.height, True))
	...
	else
	{
		if (x_visinfo->depth != 8)
			st2_fixup (x_framebuffer[current_framebuffer], 0, 0, vid.width, vid.height);
		XPutImage (x_disp, x_win, x_gc, x_framebuffer[0], 0, 0, 0, 0, vid.width, vid.height);
		XSync (x_disp, False);
	}
}
...
void SWimp_SetPalette (const unsigned char *palette)
{
	...
	for (i = 0; i < 256; i++) st2d_8to16table[i] = xlib_rgb (palette[i * 4], palette[i * 4 + 1], palette[i * 4 + 2]);
