...
{
    quake2::getInstance()->draw_chars = ref_gl_Draw_FindPic( "conchars" );
}
...
void ref_soft_Draw_Char( int x, int y, int num )
{
    ...
    row = num >> 4;
    col = num & 15;
    source = quake2::getInstance()->draw_chars->pixels[0] + ( row << 10 ) +
             ( col << 3 );
...
void ref_soft_Draw_StretchPicImplementation( int x, int y, int w, int h,
                                             image_t* pic )
{
    byte *dest, *source;
    int v, u, sv;
