...
model_t* ref_soft_Mod_LoadModel( model_t* mod, bool crash );
...
model_t* ref_soft_Mod_ForName( char* name, bool crash )
{
    ...
    {
        if ( crash )
            ri.Sys_Error( ERR_DROP, "ref_soft_Mod_NumForName: %s not found",
                          mod->name );
    ...
    switch ( LittleLong( *(unsigned*)buf ) )
    {
        case IDALIASHEADER:
            ...
            ref_soft_Mod_LoadAliasModel( mod, buf );
            break;

        case IDSPRITEHEADER:
            loadmodel->extradata = Hunk_Begin( 0x10000 );
            ref_soft_Mod_LoadSpriteModel( mod, buf );
            break;

        case IDBSPHEADER:
            loadmodel->extradata = Hunk_Begin( 0x1000000 );
            ref_soft_Mod_LoadBrushModel( mod, buf );
            break;

        default:
            ri.Sys_Error( ERR_DROP,
                          "ref_soft_Mod_NumForName: unknown fileid for %s",
                          mod->name );
            break;
    }
...
void ref_soft_Draw_GetPalette( void )
{
    ...
    ref_soft_LoadPCX( "pics/colormap.pcx", &quake2::getInstance()->vid.colormap,
                      &pal, NULL, NULL );
    if ( !quake2::getInstance()->vid.colormap )
        ri.Sys_Error( ERR_FATAL, "Couldn't load pics/colormap.pcx" );
    quake2::getInstance()->vid.alphamap =
        quake2::getInstance()->vid.colormap + 64 * 256;
...
/*
===================
ref_soft_Mod_DecompressVis
===================
*/
byte* ref_soft_Mod_DecompressVis( byte* in, model_t* model )
{
...
byte* ref_soft_Mod_ClusterPVS( int cluster, model_t* model )
{
    if ( cluster == -1 || !model->vis )
        return mod_novis;
    return ref_soft_Mod_DecompressVis(
...
/*
=================
ref_soft_Mod_LoadVisibility
=================
*/
void ref_soft_Mod_LoadVisibility( lump_t* l )
{
    int i;
...
void ref_soft_Mod_LoadVertexes( lump_t* l )
{
    ...
    if ( l->filelen % sizeof( *in ) )
        // ri.Sys_Error (ERR_DROP,"MOD_LoadBmodel: funny lump size in
        // %s",loadmodel->name);
        count = l->filelen / sizeof( *in );
    mvertex_t* out = (mvertex_t*)Hunk_Alloc(
        ( count + 8 ) * sizeof( *out ) );  // extra for skybox
...
void ref_soft_Mod_LoadEdges( lump_t* l )
{
    ...
    if ( l->filelen % sizeof( *in ) )
        ri.Sys_Error( ERR_DROP, "MOD_LoadBmodel: funny lump size in %s",
                      loadmodel->name );
    count = l->filelen / sizeof( *in );
    medge_t* out = (medge_t*)Hunk_Alloc( ( count + 13 ) *
                                         sizeof( *out ) );  // extra for skybox
...
void ref_soft_Mod_LoadTexinfo( lump_t* l )
{
    ...
    if ( l->filelen % sizeof( *in ) )
        ri.Sys_Error( ERR_DROP, "MOD_LoadBmodel: funny lump size in %s",
                      loadmodel->name );
    count = l->filelen / sizeof( *in );
    mtexinfo_t* out = (mtexinfo_t*)Hunk_Alloc(
        ( count + 6 ) * sizeof( *out ) );  // extra for skybox
...
void ref_soft_Mod_LoadFaces( lump_t* l )
{
    ...
    if ( l->filelen % sizeof( *in ) )
        ri.Sys_Error( ERR_DROP, "MOD_LoadBmodel: funny lump size in %s",
                      loadmodel->name );
    count = l->filelen / sizeof( *in );
    msurface_t* out = (msurface_t*)Hunk_Alloc(
        ( count + 6 ) * sizeof( *out ) );  // extra for skybox
...
void ref_soft_Mod_SetParent( mnode_s* node, mnode_s* parent )
{
    ...
    if ( node->contents != -1 )
        return;
    ref_soft_Mod_SetParent( node->children[0], node );
    ref_soft_Mod_SetParent( node->children[1], node );
}

/*
=================
ref_soft_Mod_LoadNodes
=================
*/
void ref_soft_Mod_LoadNodes( lump_t* l )
{
    ...
    ref_soft_Mod_SetParent( loadmodel->nodes, NULL );  // sets nodes and leafs
}

/*
=================
ref_soft_Mod_LoadLeafs
=================
*/
void ref_soft_Mod_LoadLeafs( lump_t* l )
{
    ...
    if ( l->filelen % sizeof( *in ) )
    {
        // ri.Sys_Error (ERR_DROP,"MOD_LoadBmodel: funny lump size in
        // %s",loadmodel->name);
        CUL::Assert::simple( false, "MOD_LoadBmodel: funny lump size in " +
                                        std::string( loadmodel->name ) );
    }
...
void ref_soft_Mod_LoadMarksurfaces( lump_t* l )
{
    ...
    for ( i = 0; i < count; i++ )
    {
        ...
        if ( j >= loadmodel->numsurfaces )
            ri.Sys_Error(
                ERR_DROP,
                "ref_soft_Mod_ParseMarksurfaces: bad surface number" );
...
void ref_soft_Mod_LoadSurfedges( lump_t* l )
{
    ...
    int* out = (int*)Hunk_Alloc( ( count + 24 ) *
                                 sizeof( *out ) );  // extra for skybox

    loadmodel->surfedges = out;
    loadmodel->numsurfedges = count;

    for ( int i = 0; i < count; i++ ) out[i] = LittleLong( in[i] );
}
...
void ref_soft_Mod_LoadPlanes( lump_t* l )
{
    ...
    int count = l->filelen / sizeof( *in );
    plane_s* out = (plane_s*)Hunk_Alloc( ( count + 6 ) *
                                         sizeof( *out ) );  // extra for skybox
...
void ref_soft_Mod_LoadBrushModel( model_t* mod, void* buffer )
{
    ...
    if ( i != BSPVERSION )
        ri.Sys_Error( ERR_DROP,
                      "ref_soft_Mod_LoadBrushModel: %s has wrong version "
                      "number (%i should be %i)",
    ...
    ref_soft_Mod_LoadVertexes( &header->lumps[LUMP_VERTEXES] );
    ref_soft_Mod_LoadEdges( &header->lumps[LUMP_EDGES] );
    ref_soft_Mod_LoadSurfedges( &header->lumps[LUMP_SURFEDGES] );
    ref_soft_Mod_LoadLighting( &header->lumps[LUMP_LIGHTING] );
    ref_soft_Mod_LoadPlanes( &header->lumps[LUMP_PLANES] );
    ref_soft_Mod_LoadTexinfo( &header->lumps[LUMP_TEXINFO] );
    ref_soft_Mod_LoadFaces( &header->lumps[LUMP_FACES] );
    ref_soft_Mod_LoadMarksurfaces( &header->lumps[LUMP_LEAFFACES] );
    ref_soft_Mod_LoadVisibility( &header->lumps[LUMP_VISIBILITY] );
    ref_soft_Mod_LoadLeafs( &header->lumps[LUMP_LEAFS] );
    ref_soft_Mod_LoadNodes( &header->lumps[LUMP_NODES] );
    ref_soft_Mod_LoadSubmodels( &header->lumps[LUMP_MODELS] );
    r_numvisleafs = 0;
...
void ref_soft_Mod_LoadAliasModel( model_t* mod, void* buffer )
{
    ...
    {
        mod->skins[i] = ref_soft_R_FindImage(
...
void ref_soft_Mod_LoadSpriteModel( model_t* mod, void* buffer )
{
    ...
    if ( sprout->numframes > MAX_MD2SKINS )
        ri.Sys_Error( ERR_DROP, "%s has too many frames (%i > %i)", mod->name,
    ...
    for ( int i = 0; i < sprout->numframes; i++ )
    {
        ...
        mod->skins[i] =
            ref_soft_R_FindImage( sprout->frames[i].name, it_sprite );
    }
