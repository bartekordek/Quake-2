...
extern void ref_soft_R_TransformFrustum( void );
...
void R_BeginEdgeFrame( void )
{
    ...
    if ( quake2::getInstance()->sw_draworder->value )
    {
...
void R_ScanEdges( void )
{
    ...
    basespan_p = (espan_t*)( (long)( basespans + CACHE_SIZE - 1 ) &
                             ~( CACHE_SIZE - 1 ) );
    ...
    for ( iv = r_refdef.vrect.y; iv < bottom; iv++ )
    {
        ...
        {
            ref_soft_D_DrawSurfaces();

            // clear the surface span pointers
            for ( s = &surfaces[1]; s < surface_p; s++ ) s->spans = NULL;
...
void D_CalcGradients( msurface_t* pface )
{
    ...
    d_sdivzorigin =
        p_saxis[2] * mipscale - xcenter * d_sdivzstepu - ycenter * d_sdivzstepv;
    d_tdivzorigin =
        p_taxis[2] * mipscale - xcenter * d_tdivzstepu - ycenter * d_tdivzstepv;

    VectorScale( transformed_modelorg, mipscale, p_temp1 );

    t = 0x10000 * mipscale;
    sadjust =
        ( ( fixed16_t )( DotProduct( p_temp1, p_saxis ) * 0x10000 + 0.5 ) ) -
        ( ( pface->texturemins[0] << 16 ) >> miplevel ) +
        pface->texinfo->vecs[0][3] * t;
    tadjust =
        ( ( fixed16_t )( DotProduct( p_temp1, p_taxis ) * 0x10000 + 0.5 ) ) -
    ...
    if ( pface->texinfo->flags & SURF_FLOWING )
    {
        if ( pface->texinfo->flags & SURF_WARP )
            sadjust +=
                0x10000 * ( -128 * ( ( r_newrefdef.time * 0.25 ) -
                                     (int)( r_newrefdef.time * 0.25 ) ) );
        else
            sadjust +=
                0x10000 * ( -128 * ( ( r_newrefdef.time * 0.77 ) -
                                     (int)( r_newrefdef.time * 0.77 ) ) );
    }
    // PGM
...
void D_BackgroundSurf( surf_t* s )
{
    ...
    D_FlatFillSurface(
        s, (int)quake2::getInstance()->sw_clearcolor->value & 0xFF );
    ref_soft_D_DrawZSpans( s->spans );
}
...
void D_TurbulentSurf( surf_t* s )
{
    ...
    if ( s->insubmodel )
    {
        ...
        quake2::getInstance()->currententity =
            s->entity;  // FIXME: make this passed in to
                        // R_RotateBmodel ()
        VectorSubtract( quake2::getInstance()->r_origin,
                        quake2::getInstance()->currententity->origin,
                        local_modelorg );
...
void D_SolidSurf( surf_t* s )
{
...
#else
    {
        ...
        if ( s->insubmodel )
        {
            ...
            dot = DotProduct( normal, quake2::getInstance()->vpn );
        }
        else
        {
            VectorCopy( pface->plane->normal, normal );
...
void ref_soft_D_DrawSurfaces( void )
{
    ...
    //    quake2::getInstance()->currententity = NULL;    //&r_worldentity;
    VectorSubtract( quake2::getInstance()->r_origin, vec3_origin, modelorg );
    TransformVector( modelorg, transformed_modelorg );
    VectorCopy( transformed_modelorg, world_transformed_modelorg );

    if ( !quake2::getInstance()->sw_drawflat->value )
    {
        for ( s = &surfaces[1]; s < surface_p; s++ )
        {
            if ( !s->spans )
            ...
            if ( !( s->flags & ( SURF_DRAWSKYBOX | SURF_DRAWBACKGROUND |
                                 SURF_DRAWTURB ) ) )
                D_SolidSurf( s );
