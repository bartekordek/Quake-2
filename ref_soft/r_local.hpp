...
// ref_soft_R_shared.h: general refresh-related stuff shared between the refresh and the
// driver
...
#define BMODEL_FULLY_CLIPPED    0x10 // value returned by ref_soft_R_BmodelCheckBBox ()
...
void ref_soft_R_TransformPlane (plane_t *p, float *normal, float *dist);
...
void ref_soft_R_DrawSubmodelPolygons (model_t *pmodel, int clipflags, mnode_t *topnode);
void ref_soft_R_DrawSolidClippedSubmodelPolygons (model_t *pmodel, mnode_t *topnode);

void ref_soft_R_AddPolygonEdges (emitpoint_t *pverts, int numverts, int miplevel);
...
//void ref_soft_R_DrawTriangle (finalvert_t *index0, finalvert_t *index1, finalvert_t *index2);
void ref_soft_R_AliasClipTriangle (finalvert_t *index0, finalvert_t *index1, finalvert_t *index2);
...
void ref_soft_R_ClipEdge (mvertex_t *pv0, mvertex_t *pv1, clipplane_t *clip);
void ref_soft_R_SplitEntityOnNode2 (mnode_t *node);
...
bool ref_soft_R_Init( void *hInstance, void *wndProc );
...
void ref_soft_R_BeginRegistration(const std::string& map);
model_s* ref_soft_R_RegisterModel(const std::string& name);
void ref_soft_R_EndRegistration (void);

void ref_soft_R_RenderFrame (refdef_t *fd);

image_s* ref_gl_Draw_FindPic(const std::string& name);

void    ref_soft_Draw_GetPicSize(int *w, int *h, const std::string& name);
void    ref_soft_Draw_Pic(int x, int y, const std::string& name);
void    ref_soft_Draw_StretchPic (int x, int y, int w, int h, const std::string& name);
...
void    ref_soft_Draw_TileClear (int x, int y, int w, int h, const std::string& name);
...
void ref_soft_LoadPCX(const std::string& filename, byte **pic, byte **palette, int *width, int *height);
...
void ref_soft_R_IMFlatShadedQuad( vec3_t a, vec3_t b, vec3_t c, vec3_t d, int color, float alpha );
