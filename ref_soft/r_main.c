...
void R_Register(void)
{
	...
	sw_mode->modified	= e_true;  // force us to do mode specific stuff later
	vid_gamma->modified = e_true;  // force us to rebuild the gamma table later
...
qboolean R_Init(void *hInstance, void *wndProc)
{
	...
	view_clipplanes[0].leftedge	 = e_true;
	view_clipplanes[1].rightedge = e_true;
	view_clipplanes[1].leftedge = view_clipplanes[2].leftedge = view_clipplanes[3].leftedge = e_false;
	view_clipplanes[0].rightedge = view_clipplanes[2].rightedge = view_clipplanes[3].rightedge = e_false;
...
{
	int		 i;
	qboolean translucent_entities = e_false;
...
#define NUM_BEAM_SEGS 6
	...
	for (i = 0; i < NUM_BEAM_SEGS; i++)
	{
		R_IMFlatShadedQuad(start_points[i], end_points[i], end_points[(i + 1) % NUM_BEAM_SEGS], start_points[(i + 1) % NUM_BEAM_SEGS],
...
@@@@@@@@@@@@@@@@@@@@@
*/
Q2_EXPORT refexport_t GetRefAPI (refimport_t rimp)
{
	refexport_t re;
