...
void Mod_LoadAliasModel (model_t *mod, void *buffer)
{
	...
	{
		mod->skins[i] = R_FindImage ((const char *) pheader + pheader->ofs_skins + i * MAX_SKINNAME, it_skin);
	}
}
...
struct model_s *R_RegisterModel (const char *name)
{
	...
	if (mod)
	{
		...
		if (mod->type == mod_sprite)
		{
			...
			for (i = 0; i < sprout->numframes; i++) mod->skins[i] = R_FindImage (sprout->frames[i].name, it_sprite);
		}
		...
		{
			for (i = 0; i < mod->numtexinfo; i++) mod->texinfo[i].image->registration_sequence = registration_sequence;
		}
	}
	return mod;
}
