...
model_t *ref_soft_Mod_LoadModel (model_t *mod, qboolean crash);
...
model_t *ref_soft_Mod_ForName (char *name, qboolean crash)
{
    ...
    {
        if (crash)
            ri.Sys_Error (ERR_DROP,"ref_soft_Mod_NumForName: %s not found", mod->name);
    ...
    default:
        ri.Sys_Error (ERR_DROP,"ref_soft_Mod_NumForName: unknown fileid for %s", mod->name);
        break;
    }
...
byte *ref_soft_Mod_ClusterPVS (int cluster, model_t *model)
{
    if (cluster == -1 || !model->vis)
        return mod_novis;
    return ref_soft_Mod_DecompressVis ( (byte *)model->vis + model->vis->bitofs[cluster][DVIS_PVS],
        model);
}
...
void ref_soft_Mod_LoadMarksurfaces (lump_t *l)
{
    ...
    for ( i=0 ; i<count ; i++)
    {
        ...
        if (j >= loadmodel->numsurfaces)
            ri.Sys_Error (ERR_DROP,"ref_soft_Mod_ParseMarksurfaces: bad surface number");
...
void ref_soft_Mod_LoadBrushModel (model_t *mod, void *buffer)
{
    ...
    if (i != BSPVERSION)
        ri.Sys_Error (ERR_DROP,"ref_soft_Mod_LoadBrushModel: %s has wrong version number (%i should be %i)", mod->name, i, BSPVERSION);
