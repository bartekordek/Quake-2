...
void R_RotateBmodel( void )
{
    ...
    // TODO: should really be stored with the entity instead of being
    // reconstructed
    // TODO: could cache lazily, stored in the entity
    // TODO: share work with R_SetUpAliasTransform

    // yaw
    angle = quake2::getInstance()->currententity->angles[YAW];
    ...
    // pitch
    angle = quake2::getInstance()->currententity->angles[PITCH];
    angle = angle * M_PI * 2 / 360;
    s = sin( angle );
    c = cos( angle );
    ...
    // roll
    angle = quake2::getInstance()->currententity->angles[ROLL];
    angle = angle * M_PI * 2 / 360;
    s = sin( angle );
    c = cos( angle );
...
================
*/
void R_RecursiveClipBPoly( bedge_t* pedges, mnode_s* pnode, msurface_t* psurf )
{
    bedge_t *psideedges[2], *pnextedge, *ptedge;
    int i, side, lastside;
    float dist, frac, lastdist;
    plane_s *splitplane, tplane;
    ...
    tplane.dist =
        splitplane->dist - DotProduct( r_entorigin, splitplane->normal );
    ...
    for ( ; pedges; pedges = pnextedge )
    {
        ...
        lastdist =
            DotProduct( plastvert->position, tplane.normal ) - tplane.dist;
                ...
                if ( pn->contents != CONTENTS_NODE )
                {
                    if ( pn->contents != CONTENTS_SOLID )
                    {
                        if ( r_newrefdef.areabits )
                        {
                            ...
                            if ( !( r_newrefdef.areabits[area >> 3] &
                                    ( 1 << ( area & 7 ) ) ) )
...
================
*/
void R_DrawSolidClippedSubmodelPolygons( model_t* pmodel, mnode_s* topnode )
{
    int i, j, lindex;
    vec_t dot;
    msurface_t* psurf;
...
================
*/
void R_DrawSubmodelPolygons( model_t* pmodel, int clipflags, mnode_s* topnode )
{
    int i;
    vec_t dot;
    msurface_t* psurf;
...
void ref_soft_R_RecursiveWorldNode( mnode_s* node, int clipflags )
{
    int i, c, side, *pindex;
