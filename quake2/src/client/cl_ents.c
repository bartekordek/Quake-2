...
//			Com_DPrintf("PROJ: %d CLEARED\n", cl_projectiles[i].num);
		cl_projectiles[i].present = e_false;
	}
}
...
void CL_DeltaEntity (frame_t *frame, int newnum, entity_state_t *old, int bits)
{
	...
	if (state->modelindex != ent->current.modelindex || state->modelindex2 != ent->current.modelindex2 ||
		...
		abs (state->origin[2] - ent->current.origin[2]) > 512 || state->event == EV_PLAYER_TELEPORT || state->event == EV_OTHER_TELEPORT)
	{
		ent->serverframe = -99;
	}
...
void CL_ParseFrame (void)
{
	...
	{
		cl.frame.valid	= e_true;  // uncompressed frame
		old				= NULL;
		cls.demowaiting = e_false;	// we can start recording now
	}
	else
	{
		...
		else if (cl.parse_entities - old->parse_entities > MAX_PARSE_ENTITIES - 128)
		{
			...
			cl.frame.valid = e_true;  // valid delta parse
	}
	...
	if (cl.frame.valid)
	{
		...
		cl.sound_prepped = e_true;	// can start mixing ambient sounds
	...
	for (pnum = 0; pnum < frame->num_entities; pnum++)
	{
		...
		else
		{
			...
			{  // use custom player skin
				...
				{
					if (!strncmp ((const char *) ent.skin, "players/male", 12))
					{
						ent.skin  = re.RegisterSkin ("players/male/disguise.pcx");
						ent.model = re.RegisterModel ("players/male/tris.md2");
					}
					else if (!strncmp ((const char *) ent.skin, "players/female", 14))
					{
						ent.skin  = re.RegisterSkin ("players/female/disguise.pcx");
						ent.model = re.RegisterModel ("players/female/tris.md2");
					}
					else if (!strncmp ((const char *) ent.skin, "players/cyborg", 14))
					{
...
void CL_CalcViewValues (void)
{
	...
	if (fabs (ops->pmove.origin[0] - ps->pmove.origin[0]) > 256 * 8 || abs (ops->pmove.origin[1] - ps->pmove.origin[1]) > 256 * 8 ||
	...
	{  // use predicted values
		...
		for (i = 0; i < 3; i++)
		{
			cl.refdef.vieworg[i] = cl.predicted_origin[i] + ops->viewoffset[i] + cl.lerpfrac * (ps->viewoffset[i] - ops->viewoffset[i]) -
	...
	{  // just use interpolated values
		for (i = 0; i < 3; i++)
			cl.refdef.vieworg[i] =
				...
				lerp * (ps->pmove.origin[i] * 0.125 + ps->viewoffset[i] - (ops->pmove.origin[i] * 0.125 + ops->viewoffset[i]));
	}
	...
	for (i = 0; i < 3; i++) cl.refdef.viewangles[i] += LerpAngle (ops->kick_angles[i], ps->kick_angles[i], lerp);
	...
	for (i = 0; i < 4; i++) cl.refdef.blend[i] = ps->blend[i];
