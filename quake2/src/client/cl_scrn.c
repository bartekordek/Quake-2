...
#include "quake2/client/cl_scrn.h"
...
void CL_AddNetgraph (void)
{
	...
	for (i = 0; i < cls.netchan.dropped; i++) SCR_DebugGraph (30, 0x40);

	for (i = 0; i < cl.surpressCount; i++) SCR_DebugGraph (30, 0xdf);
...
{
	S_StopAllSounds ();
	cl.sound_prepped = e_false;	 // don't play ambients
	CDAudio_Stop ();
...
char *sb_nums[2][11] = {{"num_0", "num_1", "num_2", "num_3", "num_4", "num_5", "num_6", "num_7", "num_8", "num_9", "num_minus"},
...
void SCR_TouchPics (void)
{
	...
	for (i = 0; i < 2; i++)
		for (j = 0; j < 11; j++) re.RegisterPic (sb_nums[i][j]);
