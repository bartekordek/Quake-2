...
{
	return SV_FindIndex (name, CS_MODELS, MAX_MODELS, e_true);
}

int SV_SoundIndex (const char *name)
{
	return SV_FindIndex (name, CS_SOUNDS, MAX_SOUNDS, e_true);
}

int SV_ImageIndex (const char *name)
{
	return SV_FindIndex (name, CS_IMAGES, MAX_IMAGES, e_true);
}
...
void SV_SpawnServer (const char *server, char *spawnpoint, server_state_t serverstate, qboolean attractloop, qboolean loadgame)
{
	int		 i;
	unsigned checksum;
	...
	{
		sv.models[1] = CM_LoadMap ("", e_false, &checksum);	 // no real map
	}
	else
	{
		Com_sprintf (sv.configstrings[CS_MODELS + 1], sizeof (sv.configstrings[CS_MODELS + 1]), "maps/%s.bsp", server);
		sv.models[1] = CM_LoadMap (sv.configstrings[CS_MODELS + 1], e_false, &checksum);
	}
