...
static void DrawKeyBindingFunc (void *self)
{
	...
	else
	{
		...
		if (keys[1] != -1)
		{
			...
			Menu_DrawString (a->generic.x + a->generic.parent->x + 48 + x, a->generic.y + a->generic.parent->y,
...
static void UpdateSoundQualityFunc (void *unused)
{
	if (s_options_quality_list.curvalue)
	{
		...
		Cvar_SetValue ("s_loadas8bit", e_false);
	}
	else
	{
		Cvar_SetValue ("s_khz", 11);
		Cvar_SetValue ("s_loadas8bit", e_true);
	}
...
{
	static const char *cd_music_items[]			= {"disabled", "enabled", 0};
	static const char *quality_items[]			= {"low", "high", 0};

	static const char *compatibility_items[]	= {"max compatibility", "max performance", 0};
...
static const char  *idcredits[]	  = {"+QUAKE II BY ID SOFTWARE",
									 "",
									 "+PROGRAMMING",
									 "John Carmack",
									 "John Cash",
...
static const char *xatcredits[]	  = {"+QUAKE II MISSION PACK: THE RECKONING",
									 "+BY",
...
static const char *roguecredits[] = {"+QUAKE II MISSION PACK 2: GROUND ZERO",
									 "+BY",
...
{
	static const char *difficulty_names[] = {"easy", "medium", "hard", 0};
...
void Create_Savestrings (void)
{
	...
	for (i = 0; i < MAX_SAVEGAMES; i++)
	{
		...
		if (!f)
		{
			...
			m_savevalid[i] = e_false;
		}
		else
		{
			FS_Read (m_savestrings[i], sizeof (m_savestrings[i]), f);
			fclose (f);
			m_savevalid[i] = e_true;
		}
	}
}
...
void SearchLocalGames (void)
{
	...
	m_num_servers = 0;
	for (i = 0; i < MAX_LOCAL_SERVERS; i++) strcpy (local_server_names[i], NO_SERVER_STRING);
...
void JoinServer_MenuInit (void)
{
	...
	for (i = 0; i < 8; i++) Menu_AddItem (&s_joinserver_menu, &s_joinserver_server_actions[i]);
...
void StartServerActionFunc (void *self)
{
	...
	{
		Cvar_SetValue ("deathmatch", 1);  // deathmatch is always e_true for rogue games, right?
...
void StartServer_MenuInit (void)
{
	...
	//=======
	// PGM
	static const char *dm_coop_names_rogue[] = {"deathmatch", "cooperative", "tag",
												//		"deathball",
												0};
	// PGM
	//=======
...
void PlayerConfig_MenuDraw (void)
{
	...
	if (s_pmi[s_player_model_box.curvalue].skindisplaynames)
	{
		...
		M_DrawTextBox ((refdef.x) * (320.0F / viddef.width) - 8, (viddef.height / 2) * (240.0F / viddef.height) - 77, refdef.width / 8,
					   refdef.height / 8);
		refdef.height += 4;
