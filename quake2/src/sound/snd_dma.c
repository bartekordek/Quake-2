...
#include "quake2/client/snd_loc.h"
...
==================
*/
sfx_t *S_AliasName (const char *aliasname, char *truename)
{
	sfx_t *sfx;
	char  *s;
	int	   i;
...
channel_t *S_PickChannel (int entnum, int entchannel)
{
	...
	for (ch_idx = 0; ch_idx < MAX_CHANNELS; ch_idx++)
	{
		if (entchannel != 0	 // channel 0 never overrides
			&& channels[ch_idx].entnum == entnum && channels[ch_idx].entchannel == entchannel)
...
struct sfx_s *S_RegisterSexedSound (entity_state_t *ent, char *base)
{
	...
	sfx = S_FindName (sexedFilename, e_false);
...
void S_StartSound (vec3_t origin, int entnum, int entchannel, sfx_t *sfx, float fvol, float attenuation, float timeofs)
{
	...
	if (origin)
	{
		...
		ps->fixed_origin = e_true;
	}
	else
		ps->fixed_origin = e_false;
...
void S_AddLoopSounds (void)
{
	...
	for (i = 0; i < cl.frame.num_entities; i++)
	{
		...
		ch->autosound = e_true;	 // remove next frame
		ch->sfx		  = sfx;
...
void S_RawSamples (int samples, int rate, int width, int channels, byte *data)
{
	...
	else if (channels == 2 && width == 1)
	{
		for (i = 0;; i++)
		{
			...
			dst = s_rawend & (MAX_RAW_SAMPLES - 1);
			s_rawend++;
			s_rawsamples[dst].left	= ((const char *) data)[src * 2] << 16;
			s_rawsamples[dst].right = ((const char *) data)[src * 2 + 1] << 16;
		}
	}
