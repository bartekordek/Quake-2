...
void ref_gl_GL_TexEnv( GLenum mode )
{
...
{
    if ( gl_nobind->value &&
         quake2::getInstance()->draw_chars )  // performance evaluation option
        texnum = quake2::getInstance()->draw_chars->texnum;
...
gltmode_t gl_alpha_modes[] = {
    { "default", 4 },         { "GL_RGBA", GL_RGBA },
    { "GL_RGBA8", GL_RGBA8 }, { "GL_RGB5_A1", GL_RGB5_A1 },
...
gltmode_t gl_solid_modes[] = {
    { "default", 3 },           { "GL_RGB", GL_RGB },
    { "GL_RGB8", GL_RGB8 },     { "GL_RGB5", GL_RGB5 },
    { "GL_RGB4", GL_RGB4 },     { "GL_R3_G3_B2", GL_R3_G3_B2 },
#ifdef GL_RGB2_EXT
    { "GL_RGB2", GL_RGB2_EXT },
#endif
};
...
void ref_gl_GL_ImageList_f( void )
{
    ...
    for ( i = 0, image = gltextures; i < numgltextures; i++, image++ )
    {
        ...
        ri.Con_Printf( PRINT_ALL, " %3i %3i %s: %s\n", image->upload_width,
                       image->upload_height, palstrings[image->paletted],
                       image->name );
    }
...
================
*/
void GL_LightScaleTexture( unsigned* in, int inwidth, int inheight,
                           bool only_gamma )
{
    if ( only_gamma )
    {
...
bool GL_Upload32( unsigned* data, int width, int height, bool mipmap )
{
    ...
    if ( scaled_width == width && scaled_height == height )
    {
        if ( !mipmap )
        {
                 ...
                 samples == gl_solid_format )
            {
                ...
                GL_BuildPalettedTexture( paletted_texture, (unsigned char*)data,
    ...
    {
        uploaded_paletted = true;
        GL_BuildPalettedTexture( paletted_texture, (unsigned char*)scaled,
                                 scaled_width, scaled_height );
    ...
    if ( mipmap )
    {
        ...
        while ( scaled_width > 1 || scaled_height > 1 )
        {
            ...
            {
                uploaded_paletted = true;
                GL_BuildPalettedTexture( paletted_texture,
                                         (unsigned char*)scaled, scaled_width,
                                         scaled_height );
...
bool GL_Upload8( byte* data, int width, int height, bool mipmap, bool is_sky )
{
    ...
    {
        qglTexImage2D( GL_TEXTURE_2D, 0, GL_COLOR_INDEX8_EXT, width, height, 0,
                            ...
                            imagetype_t type, int bits )
{
    ...
    nonscrap:
        ...
        if ( bits == 8 )
            ...
            image->has_alpha = GL_Upload32(
                (unsigned*)pic, width, height,
                ( image->type != it_pic && image->type != it_sky ) );
...
image_t* GL_LoadWal( char* name )
{
    ...
    image =
        ref_gl_GL_LoadPic( name, (byte*)mt + ofs, width, height, it_wall, 8 );
...
image_t* ref_gl_GL_FindImage( char* name, imagetype_t type )
{
    ...
    if ( !name )
        return NULL;  //    ri.Sys_Error (ERR_DROP, "ref_gl_GL_FindImage: NULL
                      //    name");
    len = strlen( name );
    if ( len < 5 )
        return NULL;  //    ri.Sys_Error (ERR_DROP, "ref_gl_GL_FindImage: bad
                      //    name: %s", name);
    ...
    if ( !strcmp( name + len - 4, ".pcx" ) )
    {
        ...
        if ( !pic )
            return NULL;  // ri.Sys_Error (ERR_DROP, "ref_gl_GL_FindImage: can't
                          // load %s", name);
...
{
    return ref_gl_GL_FindImage( name, it_skin );
}
