                   ...
                   float frontv[3], float backv[3] )
{
    ...
    if ( quake2::getInstance()->currententity->flags &
...
void GL_DrawAliasFrameLerp( dmdl_t* paliashdr, float backlerp )
{
    ...
    oldframe =
        (daliasframe_t*)( (byte*)paliashdr + paliashdr->ofs_frames +
                          quake2::getInstance()->currententity->oldframe *
                              paliashdr->framesize );
    ov = oldframe->verts;

    order = (int*)( (byte*)paliashdr + paliashdr->ofs_glcmds );

    //    glTranslatef (frame->translate[0], frame->translate[1],
    //    frame->translate[2]); glScalef (frame->scale[0], frame->scale[1],
    //    frame->scale[2]);

    if ( quake2::getInstance()->currententity->flags & RF_TRANSLUCENT )
        alpha = quake2::getInstance()->currententity->alpha;
    else
        alpha = 1.0;
    ...
    VectorSubtract( quake2::getInstance()->currententity->oldorigin,
                    quake2::getInstance()->currententity->origin, delta );
    AngleVectors( quake2::getInstance()->currententity->angles, vectors[0],
                  vectors[1], vectors[2] );
    ...
    //    if ( quake2::getInstance()->currententity->flags & ( RF_SHELL_RED |
...
** R_CullAliasModel
*/
static bool R_CullAliasModel( vec3_t bbox[8], entity_s* e )
{
    int i;
    vec3_t mins, maxs;
    dmdl_t* paliashdr;
...
void ref_gl_R_DrawAliasModel( entity_s* e )
{
           ...
           RF_SHELL_DOUBLE ) )
    {
        ...
        if ( ( quake2::getInstance()->currententity->flags & RF_SHELL_RED ) &&
             ( quake2::getInstance()->currententity->flags & RF_SHELL_BLUE ) &&
             ( quake2::getInstance()->currententity->flags & RF_SHELL_GREEN ) )
        {
            for ( i = 0; i < 3; i++ ) shadelight[i] = 1.0;
        }
        else if ( quake2::getInstance()->currententity->flags &
                  ( RF_SHELL_RED | RF_SHELL_BLUE | RF_SHELL_DOUBLE ) )
        {
                      ...
                      RF_SHELL_DOUBLE )
            {
                ...
                shadelight[1] = 0.7;
                  ...
                  ( RF_SHELL_HALF_DAM | RF_SHELL_GREEN ) )
        {
            VectorClear( shadelight );
            // PMM - new colors
            if ( quake2::getInstance()->currententity->flags &
            ...
            if ( quake2::getInstance()->currententity->flags & RF_SHELL_GREEN )
            {
                shadelight[1] = 1.0;
            }
        }
    ...
    //        shadelight[2])); if (max_shell_val > 0)
    //        {
    //            for (i=0; i<3; i++)
    //            {
    ...
    //        }
    // pmm
    else if ( quake2::getInstance()->currententity->flags & RF_FULLBRIGHT )
    {
        for ( i = 0; i < 3; i++ ) shadelight[i] = 1.0;
    }
    else
    ...
    if ( r_newrefdef.rdflags & RDF_IRGOGGLES &&
         quake2::getInstance()->currententity->flags & RF_IR_VISIBLE )
    {
        shadelight[0] = 1.0;
        shadelight[1] = 0.0;
        shadelight[2] = 0.0;
    ...
    shadedots = r_avertexnormal_dots[( (int)( quake2::getInstance()
                                                  ->currententity->angles[1] *
                                              ( SHADEDOT_QUANT / 360.0 ) ) ) &
                                     ( SHADEDOT_QUANT - 1 )];

    an = quake2::getInstance()->currententity->angles[1] / 180 * M_PI;
    ...
    //
    if ( quake2::getInstance()->currententity->flags &
         RF_DEPTHHACK )  // hack the depth range to prevent view model from
                         // poking into walls
         ...
         ( r_lefthand->value == 1.0F ) )
    {
        ...
        qglLoadIdentity();
        qglScalef( -1, 1, 1 );
        MYgluPerspective( r_newrefdef.fov_y,
                          (float)r_newrefdef.width / r_newrefdef.height, 4,
                          4096 );
    ...
    if ( quake2::getInstance()->currententity->skin )
        skin =
            quake2::getInstance()->currententity->skin;  // custom player skin
    else
    {
        if ( quake2::getInstance()->currententity->skinnum >= MAX_MD2SKINS )
            skin = currentmodel->skins[0];
        else
        {
            skin = currentmodel
                       ->skins[quake2::getInstance()->currententity->skinnum];
            if ( !skin )
                skin = currentmodel->skins[0];
        }
    }
    ...
    if ( ( quake2::getInstance()->currententity->frame >=
           paliashdr->num_frames ) ||
    ...
    if ( ( quake2::getInstance()->currententity->oldframe >=
           paliashdr->num_frames ) ||
         ( quake2::getInstance()->currententity->oldframe < 0 ) )
    {
...
#if 1
    if ( gl_shadows->value && !( quake2::getInstance()->currententity->flags &
