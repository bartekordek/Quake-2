...
void R_RenderDlight( dlight_t* light )
{
    ...
    {
        a = i / 16.0 * M_PI * 2;
        for( j = 0; j < 3; j++ ) v[j] = light->origin[j] + vright[j] * cos( a ) * rad + vup[j] * sin( a ) * rad;
        qglVertex3fv( v );
    }
    qglEnd();
}
...
void R_RenderDlights( void )
{
    ...
    for( i = 0; i < r_newrefdef.num_dlights; i++, l++ ) R_RenderDlight( l );
...
void R_PushDlights( void )
{
    ...
    r_dlightframecount = r_framecount + 1;  // because the count hasn't
                                            //  advanced yet for this frame
    l = r_newrefdef.dlights;
    for( i = 0; i < r_newrefdef.num_dlights; i++, l++ ) R_MarkLights( l, 1 << i, r_worldmodel->nodes );
}
...
int RecursiveLightPoint( mnode_t* node, vec3_t start, vec3_t end )
{
    ...
    for( i = 0; i < node->numsurfaces; i++, surf++ )
    {
        ...
        if( lightmap )
        {
            ...
            {
                for( i = 0; i < 3; i++ ) scale[i] = gl_modulate->value * r_newrefdef.lightstyles[surf->styles[maps]].rgb[i];
