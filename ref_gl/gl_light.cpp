...
void ref_gl_R_RenderDlight( dlight_t* light )
{
    int i, j;
    float a;
    vec3_t v;
...
void ref_gl_R_MarkLights( dlight_t* light, int bit, mnode_s* node )
{
    ...
    {
        ref_gl_R_MarkLights( light, bit, node->children[0] );
        return;
    }
    if ( dist < -light->intensity + DLIGHT_CUTOFF )
    {
...
int ref_gl_RecursiveLightPoint( mnode_s* node, vec3_t start, vec3_t end )
{
    ...
    if ( ( back < 0 ) == side )
        return ref_gl_RecursiveLightPoint( node->children[side], start, end );
    ...
    // go down front side
    r = ref_gl_RecursiveLightPoint( node->children[side], start, mid );
    if ( r >= 0 )
    ...
    // go down back side
    return ref_gl_RecursiveLightPoint( node->children[!side], mid, end );
}
...
void ref_gl_R_LightPoint( vec3_t p, vec3_t color )
{
    ...
    r = ref_gl_RecursiveLightPoint( r_worldmodel->nodes, p, end );
...
===============
*/
void ref_gl_R_AddDynamicLights( msurface_t* surf )
{
    int lnum;
    int sd, td;
...
/*
** ref_gl_R_SetCacheState
*/
void ref_gl_R_SetCacheState( msurface_t* surf )
{
    int maps;
...
void ref_gl_R_BuildLightMap( msurface_t* surf, byte* dest, int stride )
{
    ...
    if ( surf->texinfo->flags &
        ...
        ri.Sys_Error( ERR_DROP,
                      "ref_gl_R_BuildLightMap called for non-lit surface" );
    ...
    else
    {
        for ( i = 0; i < tmax; i++, dest += stride )
        {
            for ( j = 0; j < smax; j++ )
            {
                ...
                /*
                 * * So if we are doing alpha lightmaps we need to set the R, G,
                 * and B
