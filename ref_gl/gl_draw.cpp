...
void ref_gl_Draw_InitLocal( void )
{
    ...
    quake2::getInstance()->draw_chars =
        ref_gl_GL_FindImage( "pics/conchars.pcx", it_pic );
    ref_gl_GL_Bind( quake2::getInstance()->draw_chars->texnum );
...
image_t* ref_gl_Draw_FindPic( char* name )
{
    ...
    if ( name[0] != '/' && name[0] != '\\' )
    {
        ...
        gl = ref_gl_GL_FindImage( fullname, it_pic );
    }
    else
        gl = ref_gl_GL_FindImage( name + 1, it_pic );
                             ...
                             byte* data )
{
    ...
    else
    {
        ...
        qglTexImage2D( GL_TEXTURE_2D, 0, GL_COLOR_INDEX8_EXT, 256, 256, 0,
                       GL_COLOR_INDEX, GL_UNSIGNED_BYTE, image8 );
    }
