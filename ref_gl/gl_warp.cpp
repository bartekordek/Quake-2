...
void SubdividePolygon( int numverts, float* verts )
{
    ...
    glpoly_s* poly = (glpoly_s*)Hunk_Alloc( sizeof( glpoly_s ) +
                                            ( ( numverts - 4 ) + 2 ) *
...
/*
Does a water warp on the pre-fragmented glpoly_s chain
*/
void ref_gl_EmitWaterPolys( msurface_t* fa )
{
    ...
    if ( fa->texinfo->flags & SURF_FLOWING )
        scroll = -64 * ( ( r_newrefdef.time * 0.5 ) -
                         (int)( r_newrefdef.time * 0.5 ) );
    else
        scroll = 0;
...
#if !id386
            s = os + r_turbsin[(int)( ( ot * 0.125 + r_newrefdef.time ) *
                                      TURBSCALE ) &
                               255];
#else
            s = os +
                r_turbsin[Q_ftol( ( ( ot * 0.125 + rdt ) * TURBSCALE ) ) & 255];
#endif
            s += scroll;
            s *= ( 1.0 / 64 );
...
#else
            t = ot +
                r_turbsin[Q_ftol( ( ( os * 0.125 + rdt ) * TURBSCALE ) ) & 255];
#endif
            t *= ( 1.0 / 64 );
