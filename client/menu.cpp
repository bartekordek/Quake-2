...
void M_Main_Draw( void )
{
    ...
    int totalheight = 0;
    char litname[80];
    char* names[] = { "m_main_game",  "m_main_multiplayer", "m_main_options",
                      "m_main_video", "m_main_quit",        0 };
...
void M_Menu_Main_f( void ) { M_PushMenu( M_Main_Draw, M_Main_Key ); }
...
static void PlayerSetupFunc( void* unused ) { M_Menu_PlayerConfig_f(); }

static void JoinNetworkServerFunc( void* unused ) { M_Menu_JoinServer_f(); }

static void StartNetworkServerFunc( void* unused ) { M_Menu_StartServer_f(); }
...
=======================================================================
*/
char* bindnames[][2] = { { "+attack", "attack" },
...
static void KeyCursorDrawFunc( menuframework_s* menu )
{
    if ( bind_grab )
        ...
        re.DrawChar( menu->x, menu->y + menu->cursor * 9,
                     12 + ( (int)( win32_Sys_Milliseconds() / 250 ) & 1 ) );
}
...
static void CustomizeControlsFunc( void* unused ) { M_Menu_Keys_f(); }
...
{
    /*
     * * the proper way to do this is probably to have ToggleConsole_f accept a
     * parameter
     */
...
{
    static char* cd_music_items[] = { "disabled", "enabled", 0 };
    static char* quality_items[] = { "low", "high", 0 };

    static char* compatibility_items[] = { "max compatibility",
                                           "max performance", 0 };

    static char* yesno_names[] = { "no", "yes", 0 };

    static char* crosshair_names[] = { "none", "cross", "dot", "angle", 0 };
...
static char* idcredits[] = { "+QUAKE II BY ID SOFTWARE",
                             "",
                             "+PROGRAMMING",
                             "John Carmack",
                             "John Cash",
...
static char* xatcredits[] = { "+QUAKE II MISSION PACK: THE RECKONING",
                              "+BY",
...
static char* roguecredits[] = { "+QUAKE II MISSION PACK 2: GROUND ZERO",
                                "+BY",
...
static void LoadGameFunc( void* unused ) { M_Menu_LoadGame_f(); }

static void SaveGameFunc( void* unused ) { M_Menu_SaveGame_f(); }

static void CreditsFunc( void* unused ) { M_Menu_Credits_f(); }
...
void AddressBookFunc( void* self ) { M_Menu_AddressBook_f(); }
...
void SearchLocalGamesFunc( void* self ) { SearchLocalGames(); }
...
void RulesChangeFunc( void* self )
{
    ...
    else if ( s_rules_box.curvalue == 1 )  // coop                // PGM
    {
        ...
        if ( std::stoi( s_maxclients_field.buffer ) > 4 )
        {
...
void StartServerActionFunc( void* self )
{
    ...
    Cvar_Set( "hostname", toChar( s_hostname_field.buffer ) );
    ...
    else
    {
        Cvar_SetValue(
            "deathmatch",
            1 );  // deathmatch is always true for rogue games, right?
        Cvar_SetValue( "coop", 0 );  // FIXME - this might need to depend on
                                     // which game we're running
...
{
    static char* dm_coop_names[] = { "deathmatch", "cooperative", 0 };
    //=======
    // PGM
    static char* dm_coop_names_rogue[] = { "deathmatch", "cooperative", "tag",
                                           //        "deathball",
                                           0 };
    // PGM
    //=======
    ...
    else
    {
        ...
        fseek( fp, 0, SEEK_SET );
#endif
        buffer = new char( length );  // malloc( length );
    ...
    for ( i = 0; i < nummaps; i++ )
    {
        ...
        l = strlen( shortname );
        for ( j = 0; j < l; j++ ) shortname[j] = toupper( shortname[j] );
        strcpy( longname, COM_Parse( &s ) );
        // Com_sprintf( scratch, sizeof( scratch ), "%s\n%s", longname,
        // shortname );

        mapnames[i] = new char[strlen( scratch ) + 1];
    ...
    strcpy( toChar( s_timelimit_field.buffer ),
            Cvar_VariableString( "timelimit" ) );
    ...
    strcpy( toChar( s_fraglimit_field.buffer ),
            Cvar_VariableString( "fraglimit" ) );
...
void StartServer_MenuDraw( void ) { Menu_Draw( &s_startserver_menu ); }
...
const char* StartServer_MenuKey( int key )
{
    if ( key == K_ESCAPE )
    {
        if ( mapnames )
        {
            ...
            for ( i = 0; i < nummaps; i++ ) free( mapnames[i] );
            free( mapnames );
        }
        mapnames = 0;
        nummaps = 0;
...
setvalue:
    ...
    // Com_sprintf( dmoptions_statusbar, sizeof( dmoptions_statusbar ), "dmflags
    // = %d", flags );
}
...
void DMOptions_MenuDraw( void ) { Menu_Draw( &s_dmoptions_menu ); }
...
void DownloadOptions_MenuDraw( void ) { Menu_Draw( &s_downloadoptions_menu ); }
...
const char* AddressBook_MenuKey( int key )
{
    if ( key == K_ESCAPE )
    {
        ...
        for ( index = 0; index < NUM_ADDRESSBOOK_ENTRIES; index++ )
        {
            ...
            Cvar_Set( buffer, toChar( s_addressbook_fields[index].buffer ) );
        }
    }
...
static char* rate_names[] = { "28.8 Modem",
                              "33.6 Modem",
                              "Single ISDN",
                              "Dual ISDN/Cable",
                              "T1/LAN",
...
void DownloadOptionsFunc( void* self ) { M_Menu_DownloadOptions_f(); }
...
static bool PlayerConfig_ScanDirectories( void )
{
    ...
    do
    {
        ...
        if ( ( dirnames = FS_ListFiles( findname, &ndirs, SFF_SUBDIR, 0 ) ) !=
             0 )
            break;
    } while ( path );
    ...
    for ( i = 0; i < npms; i++ )
    {
        ...
        strcpy( scratch, dirnames[i] );
        strcat( scratch, "/*.pcx" );
        pcxnames = FS_ListFiles( scratch, &npcxfiles, 0,
                                 SFF_SUBDIR | SFF_HIDDEN | SFF_SYSTEM );
...
const char* PlayerConfig_MenuKey( int key )
{
    ...
    if ( key == K_ESCAPE )
    {
        ...
        Cvar_Set( "name", toChar( s_player_name_field.buffer ) );
