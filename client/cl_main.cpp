...
void CL_Record_f( void )
{
    ...
    //
    SZ_Init( &buf, (byte*)buf_data.c_str(), buf_data.length() );
...
void CL_Connect_f( void )
{
    ...
    if ( Com_ServerState() )
    {  // if running a local server, kill it and
       // reissue
...
void CL_Rcon_f( void )
{
    ...
    NET_SendPacket( netsrc_t::NS_CLIENT, message.length() + 1,
                    (char*)message.c_str(), to );
}
...
void CL_ConnectionlessPacket( void )
{
    ...
    if ( !strcmp( c, "client_connect" ) )
    {
        ...
        Netchan_Setup( netsrc_t::NS_CLIENT, &cls.netchan, net_from,
                       cls.quakePort );
        MSG_WriteChar( &cls.netchan.message, clc_stringcmd );
        MSG_WriteString( &cls.netchan.message, "new" );
        cls.state = ca_connected;
    ...
    if ( !strcmp( c, "echo" ) )
    {
        Netchan_OutOfBandPrint( netsrc_t::NS_CLIENT, net_from, "%s",
                                Cmd_Argv( 1 ) );
        return;
    }
...
void CL_RequestNextDownload( void )
{
    ...
    if ( !quake2::getInstance()->allow_download->value &&
    ...
    {
        if ( quake2::getInstance()->allow_download_players->value )
        {
...
void CL_FixCvarCheats( void )
{
    ...
    if ( !numcheatvars )
    {
        while ( !cheatvars[numcheatvars].name.empty() )
        {
            cheatvars[numcheatvars].var =
                Cvar_Get( toChar( cheatvars[numcheatvars].name ),
                          toChar( cheatvars[numcheatvars].value ), 0 );
            ++numcheatvars;
        }
    }
    ...
    for ( i = 0, var = cheatvars; i < numcheatvars; i++, var++ )
    {
        ...
        {
            Cvar_Set( toChar( var->name ), toChar( var->value ) );
        }
    }
}
