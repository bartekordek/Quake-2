...
void CL_Stop_f( void )
{
    ...
    {
        Com_Printf_C( "Not recording a demo.\n" );
        return;
    }
...
void CL_Record_f( void )
{
    ...
    if( Cmd_Argc() != 2 )
    {
        Com_Printf_C( "record <demoname>\n" );
        return;
    }

    if( cls.demorecording )
    {
        Com_Printf_C( "Already recording.\n" );
        return;
    }

    if( cls.state != ca_active )
    {
        Com_Printf_C( "You must be in a level to record.\n" );
        return;
    }
    ...
    Com_Printf_C( "recording to %s.\n", name );
    FS_CreatePath( name );
    cls.demofile = fopen( name, "wb" );
    if( !cls.demofile )
    {
        Com_Printf_C( "ERROR: couldn't open.\n" );
        return;
    }
...
void Cmd_ForwardToServer( void )
{
    ...
    {
        Com_Printf_C( "Unknown command \"%s\"\n", cmd );
        return;
    }
...
void CL_ForwardToServer_f( void )
{
    ...
    {
        Com_Printf_C( "Can't \"%s\", not connected\n", Cmd_Argv( 0 ) );
        return;
    }
...
void CL_SendConnectPacket( void )
{
    ...
    {
        Com_Printf_C( "Bad server address\n" );
        cls.connect_time = 0;
        return;
    }
    if( adr.port == 0 )
...
void CL_Connect_f( void )
{
    ...
    if( Cmd_Argc() != 2 )
    {
        Com_Printf_C( "usage: connect <server>\n" );
        return;
    }
...
void CL_Disconnect( void )
{
    ...
    if( cl_timedemo && cl_timedemo->value )
    {
        ...
        time = win32_Sys_Milliseconds() - cl.timedemo_start;
        if( time > 0 )
            Com_Printf_C( "%i frames, %3.1f seconds: %3.1f fps\n",
...
void CL_Packet_f( void )
{
    ...
    if( Cmd_Argc() != 3 )
    {
        Com_Printf_C( "packet <destination> <contents>\n" );
        return;
    }
...
{
    // ZOID
    // if we are downloading, we don't change!  This so we don't suddenly stop
    // downloading a map
    if( cls.download )
        return;
...
void CL_PingServers_f( void )
{
    ...
    Com_Printf_C( "pinging broadcast...\n" );
    ...
    for( i = 0; i < 16; i++ )
    {
        ...
        Com_Printf_C( "pinging %s...\n", adrstring );
        if( !NET_StringToAdr( adrstring, &adr ) )
        {
            Com_Printf_C( "Bad address: %s\n", adrstring );
            continue;
        }
        if( !adr.port )
            adr.port = BigShort( PORT_SERVER );
...
void CL_ConnectionlessPacket( void )
{
    ...
    if( !strcmp( c, "client_connect" ) )
    {
        if( cls.state == ca_connected )
        {
            Com_Printf_C( "Dup connect received.  Ignored.\n" );
            return;
        }
    ...
    if( !strcmp( c, "cmd" ) )
    {
        ...
        {
            Com_Printf_C( "Command packet from remote host.  Ignored.\n" );
            return;
        }
#ifdef _WIN32
...
void CL_DumpPackets( void )
{
    ...
    {
        Com_Printf_C( "dumnping a packet\n" );
    }
}
...
{
    Com_Printf_C( "User info settings:\n" );
        ...
        precache_check < CS_PLAYERSKINS + MAX_CLIENTS * PLAYER_MULT )
    {
        if( allow_download_players->value )
        {
            while( precache_check < CS_PLAYERSKINS + MAX_CLIENTS * PLAYER_MULT )
            {
                ...
                if( ( p = strchr( cl.configstrings[CS_PLAYERSKINS + i],
                                  '\\' ) ) != NULL )
                    p++;
                else
                ...
                switch( n )
                {
                    ...
                    case 3:  // skin
                        Com_sprintf( fn, sizeof( fn ), "players/%s/%s.pcx",
                                     model, skin );
                        if( !CL_CheckOrDownloadFile( fn ) )
                        {
                            precache_check =
...
{
    cls.state = ca_disconnected;
    cls.realtime = win32_Sys_Milliseconds();
    ...
    cl_stereo_separation =
        Cvar_Get( "cl_stereo_separation", "0.4", CVAR_ARCHIVE );
    ...
    gender->modified =
        false;  // clear this so we know when user sets it manually
    ...
    //     Cmd_AddCommand ("packet", CL_Packet_f); // this is dangerous to leave
    //     in
...
void CL_WriteConfiguration( void )
{
    ...
    if( !f )
    {
        Com_Printf_C( "Couldn't write config.cfg.\n" );
        return;
    }
...
void CL_Frame( int msec )
{
    ...
    if( msec > 5000 )
        cls.netchan.last_received = win32_Sys_Milliseconds();
    ...
    if( host_speeds->value )
        time_before_ref = win32_Sys_Milliseconds();
    SCR_UpdateScreen();
    if( host_speeds->value )
        time_after_ref = win32_Sys_Milliseconds();
    ...
    if( log_stats->value )
    {
        if( cls.state == ca_active )
        {
            if( !lasttimecalled )
            {
                lasttimecalled = win32_Sys_Milliseconds();
                if( log_stats_file )
                    fprintf( log_stats_file, "0\n" );
            }
            else
            {
                int now = win32_Sys_Milliseconds();
