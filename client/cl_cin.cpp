...
void SCR_LoadPCX( std::string& filename, byte** pic, byte** palette, int* width,
                  int* height )
{
    byte* raw = nullptr;
    pcx_t* pcx;
    ...
    *pic = NULL;
    ...
    // load the file
    int len = FS_LoadFile( toChar( filename ), (void**)&raw );
    if ( !raw )
        return;  // Com_Printf_G ("Bad pcx file %s\n", filename);
...
void Huff1TableInit( void )
{
    ...
    for ( prev = 0; prev < 256; prev++ )
    {
        ...
        for ( j = 0; j < 256; j++ ) cin.h_count[j] = counts[j];
...
cblock_t Huff1Decompress( cblock_t in )
{
    ...
    {
        Com_Printf_G( "Decompression overread by %i",
...
void SCR_RunCinematic( void )
{
    ...
    {
        Com_Printf_G( "Dropped frame: %i > %i\n", frame,
                      cl.cinematicframe + 1 );
        cl.cinematictime = cls.realtime - cl.cinematicframe * 1000 / 14;
    }
    if ( cin.pic )
...
void SCR_PlayCinematic( const std::string& arg )
{
    ...
    cl.cinematicframe = 0;
    if ( arg.find( ".pcx" ) != std::string::npos )
    {  // static pcx image
        name = "pics/" + arg;
    ...
    name = "videeo/" + arg;
    FS_FOpenFile( toChar( name ), &cl.cinematic_file );
    if ( !cl.cinematic_file )
    {
