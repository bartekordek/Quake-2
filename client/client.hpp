...
typedef struct
{
    ...
    entity_state
        prev;  // will always be valid, but might just be a copy of current
...
#define MAX_CLIENTWEAPONMODELS \
    20  // PGM -- upped from 16 to fit the chainfist vwep
...
typedef struct
{
    ...
    short predicted_origins[CMD_BACKUP]
                           [3];  // for debug comparing against server
    ...
    // sent to the server each frame.  It is cleared to 0 upon entering each
    // level. the server sends a delta each frame which is added to the locally
...
void CL_SmokeTrail( vec3_t start, vec3_t end, int colorStart, int colorRun,
                    int spacing );
void CL_Flashlight( int ent, vec3_t pos );
void CL_ForceWall( vec3_t start, vec3_t end, int color );
void CL_FlameEffects( centity_t* ent, vec3_t origin );
void CL_GenericParticleEffect( vec3_t org, vec3_t dir, int color, int count,
                               int numcolors, int dirspread, float alphavel );
...
void CL_ParticleSmokeEffect( vec3_t org, vec3_t dir, int color, int count,
                             int magnitude );
...
void CL_SendMove( usercmd_s* cmd );
...
void CL_WriteToServer( usercmd_s* cmd );
