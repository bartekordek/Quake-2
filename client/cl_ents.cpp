...
==================
*/
void CL_ParseDelta( entity_state* from, entity_state* to, int number,
                    unsigned bits )
{
...
void CL_DeltaEntity( frame_t* frame, int newnum, entity_state* old, int bits )
{
    ...
    if ( state->modelindex != ent->current.modelindex ||
         ...
         Math::abs( state->origin[0] - ent->current.origin[0] ) > 512.f ||
         Math::abs( state->origin[1] - ent->current.origin[1] ) > 512.f ||
         Math::abs( state->origin[2] - ent->current.origin[2] ) > 512.f ||
...
void CL_ParsePacketEntities( frame_t* oldframe, frame_t* newframe )
{
    ...
    else
    {
        ...
        {
            oldstate =
                &cl_parse_entities[( oldframe->parse_entities + oldindex ) &
...
model_s* S_RegisterSexedModel( entity_state* ent, const std::string& base )
{
    ...
    mdl = re.RegisterModel( toChar( buffer ) );
    if ( !mdl )
    {
...
void CL_AddPacketEntities( frame_t* frame )
{
    ...
    for ( pnum = 0; pnum < frame->num_entities; pnum++ )
    {
        s1 = &cl_parse_entities[( frame->parse_entities + pnum ) &
                                ( MAX_PARSE_ENTITIES - 1 )];
        ...
        else
        {
            ...
            {  // use custom player skin
                ...
                if ( renderfx & RF_USE_DISGUISE )
                {
                    ...
                    else if ( !strncmp( (char*)ent.skin, "players/female",
                                        14 ) )
                    {
                        ent.skin =
                    ...
                    else if ( !strncmp( (char*)ent.skin, "players/cyborg",
                                        14 ) )
                    {
                        ent.skin =
        ...
        if ( s1->modelindex2 )
        {
            ...
            // PGM - hack to allow translucent linked models (defender sphere's
            // shell)
            //        set the high bit 0x80 on modelindex2 to enable
            //        translucency
        ...
        if ( ( effects & ~EF_ROTATE ) )
        {
            ...
            {
                //                CL_BlasterTrail (cent->lerp_origin,
                //                ent.origin);
                // PGM
...
void CL_AddViewWeapon( player_state* ps, player_state* ops )
{
    ...
    for ( i = 0; i < 3; i++ )
    {
        ...
        gun.angles[i] =
            ...
            LerpAngle( ops->gunangles[i], ps->gunangles[i], cl.lerpfrac );
    }
    ...
    {  // just use interpolated values
        for ( i = 0; i < 3; i++ )
            cl.refdef.vieworg[i] =
                ...
                lerp *
                    ( ps->pmove.origin[i] * 0.125 + ps->viewoffset[i] -
                      ( ops->pmove.origin[i] * 0.125 + ops->viewoffset[i] ) );
    }

    // if not running a demo or on a locked frame, add the local angle movement
    if ( cl.frame.playerstate.pmove.pm_type < pmtype_t::PM_DEAD )
    {  // use predicted values
        for ( i = 0; i < 3; i++ )
