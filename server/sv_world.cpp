...
// (type *)STRUCT_FROM_LINK(link_s *link, type, member)
...
int SV_HullForEntity( edict* ent );

// ClearLink is used for new headnodes
void ClearLink( link_s* l ) { l->prev = l->next = l; }

void RemoveLink( link_s* l )
{
    l->next->prev = l->prev;
    l->prev->next = l->next;
}

void InsertLinkBefore( link_s* l, link_s* before )
{
    l->next = before;
    l->prev = before->prev;
    l->prev->next = l;
...
void SV_LinkEdict( edict* ent )
{
    ...
    if ( ent == quake2::getInstance()->ge.edicts )
...
================
*/
int SV_AreaEdicts( vec3_t mins, vec3_t maxs, edict** list, int maxcount,
                   int areatype )
{
    area_mins = mins;
    area_maxs = maxs;
...
int SV_PointContents( vec3_t p )
{
    ...
    for ( i = 0; i < num; i++ )
    {
        ...
        c2 = CM_TransformedPointContents( p, headnode, hit->s.origin,
                                          hit->s.angles );
...
{
    int i, num;
    edict *touchlist[MAX_EDICTS], *touch;
    trace_t trace;
    int headnode;
    float* angles;
    ...
    for ( i = 0; i < num; i++ )
    {
        ...
        if ( touch->svflags & SVF_MONSTER )
            trace = CM_TransformedBoxTrace(
                clip->start, clip->end, clip->mins2, clip->maxs2, headnode,
                clip->contentmask, touch->s.origin, angles );
        else
            trace = CM_TransformedBoxTrace(
                clip->start, clip->end, clip->mins, clip->maxs, headnode,
                clip->contentmask, touch->s.origin, angles );
...
==================
*/
void SV_TraceBounds( vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end,
...
trace_t SV_Trace( vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end,
                  edict* passedict, int contentmask )
{
    moveclip_t clip;
    ...
    clip.trace.ent = quake2::getInstance()->ge.edicts;
