...
void SV_Baselines_f (void)
{
	...
	while (sv_client->netchan.message.cursize < MAX_MSGLEN / 2 && start < MAX_EDICTS)
	{
		...
		if (base->modelindex || base->sound || base->effects)
		{
			...
			MSG_WriteDeltaEntity (&nullstate, base, &sv_client->netchan.message, e_true, e_true);
		}
		start++;
	}
	...
	while (1)
	{
		...
		switch (c)
		{
			...
			case clc_move:
				...
				calculatedChecksum = COM_BlockSequenceCRCByte (net_message.data + checksumIndex + 1,
															   net_message.readcount - checksumIndex - 1, cl->netchan.incoming_sequence);
