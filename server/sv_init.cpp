...
int SV_FindIndex( char* name, int start, int max, bool create )
{
    ...
    {  // send the update to everyone
        ...
        SV_Multicast( vec3_origin, multicast_t::MULTICAST_ALL_R );
    }
...
void SV_CreateBaseline( void )
{
    ...
    for ( entnum = 1; entnum < quake2::getInstance()->quake2::getInstance()->ge.num_edicts; entnum++ )
    {
...
void SV_CheckForSavegame( void )
{
    ...
    {  // coming back to a level after being in a different
        ...
        for ( i = 0; i < 100; i++ ) quake2::getInstance()->quake2::getInstance()->ge.RunFrame();
                     ...
                     bool attractloop, bool loadgame )
{
    ...
    {
        // Com_sprintf
        // (sv.configstrings[CS_MODELS+1],sizeof(sv.configstrings[CS_MODELS+1]),"maps/%s.bsp",
        // server);
        sv.models[1] =
            CM_LoadMap( sv.configstrings[CS_MODELS + 1], false, &checksum );
    }
    // Com_sprintf
    // (sv.configstrings[CS_MAPCHECKSUM],sizeof(sv.configstrings[CS_MAPCHECKSUM]),"%i",
    // checksum);
    ...
    for ( i = 1; i < CM_NumInlineModels(); i++ )
    {
        ...
        // sizeof(sv.configstrings[CS_MODELS+1+i]),"*%i", i);
        sv.models[i + 1] =
            CM_InlineModel( sv.configstrings[CS_MODELS + 1 + i] );
    }
    ...
    quake2::getInstance()->quake2::getInstance()->ge.SpawnEntities( sv.name, CM_EntityString(),
                                              spawnpoint );
...
void SV_InitGame( void )
{
    ...
    // quake2::getInstance()->dedicated servers are can't be single player and are usually DM
    ...
    if ( Cvar_VariableValue( "deathmatch" ) )
    {
        if ( maxclients->value <= 1 )
            ...
            Cvar_FullSet( "maxclients", va( "%i", MAX_CLIENTS ),
...
#ifdef COPYPROTECT
        if ( !sv.attractloop && !quake2::getInstance()->dedicated->value )
            Sys_CopyProtect();
#endif
    }
    ...
    svs.client_entities = (entity_state*)Z_Malloc( sizeof( entity_state ) *
