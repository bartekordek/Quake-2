...
void SV_SetMaster_f( void )
{
    ...
    // only quake2::getInstance()->dedicated servers send heartbeats
    if ( !quake2::getInstance()->dedicated->value )
    {
        Com_Printf_G(
            "Only quake2::getInstance()->dedicated servers use masters.\n" );
        return;
    }
    ...
    for ( i = 1; i < Cmd_Argc(); i++ )
    {
        ...
        Com_Printf_G( "Master server at %s\n",
                      NET_AdrToString( master_adr[slot] ) );

        Com_Printf_G( "Sending a ping.\n" );

        Netchan_OutOfBandPrint( netsrc_t::NS_SERVER, master_adr[slot], "ping" );
...
bool SV_SetPlayer( void )
{
    ...
    if ( s[0] >= '0' && s[0] <= '9' )
    {
        ...
        {
            Com_Printf_G( "Bad client slot: %i\n", idnum );
            return false;
        }
    ...
    Com_Printf_G( "Userid %s is not on the server\n", s );
    return false;
}
...
void SV_WipeSavegame( char* savename )
{
    ...
    Sys_FindClose();
    // Com_sprintf (name, sizeof(name), "%s/save/%s/*.sv2", FS_Gamedir (),
    // savename);
    s = Sys_FindFirst( name, 0, 0 );
    while ( s )
    {
...
void SV_CopySaveGame( char* src, char* dst )
{
    ...
    // Com_sprintf (name, sizeof(name), "%s/save/%s/server.ssv", FS_Gamedir(),
    // src); Com_sprintf (name2, sizeof(name2), "%s/save/%s/server.ssv",
    // FS_Gamedir(), dst);
    FS_CreatePath( name2 );
    CopyFile( name, name2 );

    // Com_sprintf (name, sizeof(name), "%s/save/%s/game.ssv", FS_Gamedir(),
    // src); Com_sprintf (name2, sizeof(name2), "%s/save/%s/game.ssv",
    // FS_Gamedir(), dst);
    CopyFile( name, name2 );
    ...
    while ( found )
    {
        ...
        // Com_sprintf (name2, sizeof(name2), "%s/save/%s/%s", FS_Gamedir(),
        // dst, found+len);
        CopyFile( name, name2 );
...
void SV_WriteLevelFile( void )
{
    ...
    // Com_sprintf (name, sizeof(name), "%s/save/current/%s.sv2", FS_Gamedir(),
    // sv.name);
    f = fopen( name, "wb" );
    if ( !f )
    {
    ...
    // sv.name);
    quake2::getInstance()->ge.WriteLevel( name );
}
...
void SV_ReadLevelFile( void )
{
    ...
    // Com_sprintf (name, sizeof(name), "%s/save/current/%s.sav", FS_Gamedir(),
    // sv.name);
    quake2::getInstance()->ge.ReadLevel( name );
}
...
void SV_WriteServerFile( bool autosave )
{
    ...
    // Com_sprintf (name, sizeof(name), "%s/save/current/server.ssv",
    // FS_Gamedir());
    f = fopen( name, "wb" );
    if ( !f )
    {
        Com_Printf_G( "Couldn't write %s\n", name );
        return;
    }
    ...
    for ( var = cvar_vars; var; var = var->next )
    {
        ...
        //{
        //    Com_Printf_G ("Cvar too long: %s = %s\n", var->name, var->string);
        //    continue;
        //}
        memset( name, 0, sizeof( name ) );
        memset( string, 0, sizeof( string ) );
        strcpy( name, var->name.c_str() );
        strcpy( string, var->string.c_str() );
    ...
    // FS_Gamedir());
    quake2::getInstance()->ge.WriteGame( name, autosave );
}
...
void SV_ReadServerFile( void )
{
    ...
    // Com_sprintf (name, sizeof(name), "%s/save/current/game.ssv",
    // FS_Gamedir());
    quake2::getInstance()->ge.ReadGame( name );
}
...
void SV_GameMap_f( void )
{
    ...
    if ( Cmd_Argc() != 2 )
    {
        Com_Printf_G( "USAGE: gamemap <map>\n" );
        return;
    }
...
void SV_Loadgame_f( void )
{
    ...
    if ( Cmd_Argc() != 2 )
    {
        Com_Printf_G( "USAGE: loadgame <directory>\n" );
        return;
    }

    Com_Printf_G( "Loading game...\n" );

    dir = Cmd_Argv( 1 );
    if ( strstr( dir, ".." ) || strstr( dir, "/" ) || strstr( dir, "\\" ) )
    {
        Com_Printf_G( "Bad savedir.\n" );
    }
    ...
    if ( !f )
    {
        Com_Printf_G( "No such savegame: %s\n", name );
        return;
    }
    fclose( f );
...
void SV_Savegame_f( void )
{
    ...
    if ( sv.state != ss_game )
    {
        Com_Printf_G( "You must be in a game to save.\n" );
        return;
    }

    if ( Cmd_Argc() != 2 )
    {
        Com_Printf_G( "USAGE: savegame <directory>\n" );
        return;
    }

    if ( Cvar_VariableValue( "deathmatch" ) )
    {
        Com_Printf_G( "Can't savegame in a deathmatch\n" );
        return;
    }

    if ( !strcmp( Cmd_Argv( 1 ), "current" ) )
    {
        Com_Printf_G( "Can't save to 'current'\n" );
        return;
    }
...
void SV_Kick_f( void )
{
    if ( !svs.initialized )
    {
        Com_Printf_G( "No server running.\n" );
        return;
    }

    if ( Cmd_Argc() != 2 )
    {
        Com_Printf_G( "Usage: kick <userid>\n" );
        return;
    }
...
void SV_Status_f( void )
{
    ...
    for ( i = 0, cl = svs.clients; i < maxclients->value; i++, cl++ )
    {
        ...
        else
        {
            ...
            Com_Printf_G( "%4i ", ping );
        }

        Com_Printf_G( "%s", cl->name );
        l = 16 - strlen( cl->name );
        for ( j = 0; j < l; j++ ) Com_Printf_G( " " );

        Com_Printf_G( "%7i ", svs.realtime - cl->lastmessage );
        ...
        Com_Printf_G( "%5i", cl->netchan.qport );
...
{
    Com_Printf_G( "Server info settings:\n" );
...
void SV_DumpUser_f( void )
{
    if ( Cmd_Argc() != 2 )
    {
        Com_Printf_G( "Usage: info <userid>\n" );
        return;
    }
...
void SV_ServerRecord_f( void )
{
    ...
    if ( Cmd_Argc() != 2 )
    {
        Com_Printf_G( "serverrecord <demoname>\n" );
        return;
    }
...
void SV_ServerStop_f( void )
{
    if ( !svs.demofile )
    {
        Com_Printf_G( "Not doing a serverrecord.\n" );
        return;
    }
    fclose( svs.demofile );
    svs.demofile = NULL;
    Com_Printf_G( "Recording completed.\n" );
}
...
void SV_ServerCommand_f( void )
{
    ...
    if ( false )
    {
        Com_Printf_G( "No game loaded.\n" );
        return;
    }

    quake2::getInstance()->ge.ServerCommand();
}
