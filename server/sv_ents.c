...
void SV_EmitPacketEntities (client_frame_t *from, client_frame_t *to, sizebuf_t *msg)
{
	...
	while (newindex < to->num_entities || oldindex < from_num_entities)
	{
		...
		{	// delta update from old position
			// because the force parm is e_false, this will not result
			// in any bytes being emited if the entity has not changed at all
			// note that players are always 'newentities', this updates their oldorigin always
			// and prevents warping
			MSG_WriteDeltaEntity (oldent, newent, msg, e_false, newent->number <= maxclients->value);
			oldindex++;
			newindex++;
			continue;
		}
		...
		{	// this is a new entity, send it from the baseline
			MSG_WriteDeltaEntity (&sv.baselines[newnum], newent, msg, e_true, e_true);
			newindex++;
			continue;
		}
...
void SV_RecordDemoMessage (void)
{
	...
	while (e < ge->num_edicts) 
	{
		...
		if (ent->inuse &&
			...
			MSG_WriteDeltaEntity (&nostate, &ent->s, &buf, e_false, e_true);
