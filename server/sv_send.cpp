...
void SV_BroadcastPrintf( int level, char* fmt, ... )
{
    ...
    if ( quake2::getInstance()->dedicated->value )
    {
        ...
        // mask off high bits
        for ( i = 0; i < 1023 && string[i]; i++ ) copy[i] = string[i] & 127;
        copy[i] = 0;
        Com_Printf_G( "%s", copy );
    }
...
multicast_t::MULTICAST_ALL    same as broadcast (origin can be NULL)
multicast_t::MULTICAST_PVS    send to clients potentially visible from org
multicast_t::MULTICAST_PHS    send to clients potentially hearable from org
=================
*/
void SV_Multicast( vec3_t origin, multicast_t to )
{
    ...
    if ( to != multicast_t::MULTICAST_ALL_R &&
         to != multicast_t::MULTICAST_ALL )
    {
    ...
    if ( svs.demofile )
        SZ_Write( &svs.demo_multicast, sv.multicast.data,
...
==================
*/
void SV_StartSound( vec3_t origin, edict* entity, int channel, int soundindex,
                    float volume, float attenuation, float timeofs )
{
    ...
    if ( !origin )
    {
        ...
        {
            for ( i = 0; i < 3; i++ )
                origin_v[i] = entity->s.origin[i] +
                              0.5 * ( entity->mins[i] + entity->maxs[i] );
        }
        else
        {
...
bool SV_SendClientDatagram( client_t* client )
{
    ...
    // send over all the relevant entity_state
    // and the player_state
    ...
    if ( client->datagram.overflowed )
        Com_Printf_G( "WARNING: datagram overflowed for %s\n", client->name );
    else
    ...
    {  // must have room left for the packet header
        Com_Printf_G( "WARNING: msg overflowed for %s\n", client->name );
        SZ_Clear( &msg );
    }
