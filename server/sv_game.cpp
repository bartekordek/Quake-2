...
{
    return ( (edict*)( (byte*)quake2::getInstance()->quake2::getInstance()->ge.edicts +
                       quake2::getInstance()->quake2::getInstance()->ge.edict_size * ( n ) ) );
}

int Edict_idx( edict* e )
{
    return ( ( (byte*)( e ) - (byte*)quake2::getInstance()->quake2::getInstance()->ge.edicts ) /
             quake2::getInstance()->quake2::getInstance()->ge.edict_size );
}
...
{
    // TODO: IF g does not exist.
    // if ( !ge )
    //     return;
    quake2::getInstance()->quake2::getInstance()->ge.Shutdown();
#ifdef _WIN32
...
void SV_InitGameProgs( void )
{
    ...
    // quake2::getInstance()->ge =
    //     (game_export_t*)malloc( sizeof( game_import ) );

    quake2::getInstance()->quake2::getInstance()->ge.Init();

    // ge = (game_export_t *)Sys_GetGameAPI (&import);

    // if (!ge)
    //    Com_Error (ERR_DROP, "failed to load game DLL");
    // if (quake2::getInstance()->ge.apiversion != GAME_API_VERSION)
    //    Com_Error (ERR_DROP, "game is version %i, not %i", quake2::getInstance()->ge.apiversion,
    //    GAME_API_VERSION);

    // quake2::getInstance()->ge.Init ();
}
