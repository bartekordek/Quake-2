...
edict    *projectiles[MAX_PROJECTILES];
int        numprojs;
cvar  *sv_projectiles;

bool SV_AddProjectileUpdate (edict *ent)
{
    if (!sv_projectiles)
...
Writes a delta update of an entity_state list to the message.
=============
*/
void SV_EmitPacketEntities( client_frame_t* from, client_frame_t* to,
                            sizebuf_t* msg )
{
    entity_state *oldent, *newent;
    ...
    while ( newindex < to->num_entities || oldindex < from_num_entities )
    {
        ...
        {
            newent = &svs.client_entities[( to->first_entity + newindex ) %
        ...
        {
            oldent = &svs.client_entities[( from->first_entity + oldindex ) %
                                          svs.num_client_entities];
                                  ...
                                  sizebuf_t* msg )
{
    ...
    //
    // write the pmove_state
    //
    if ( pflags & PS_M_TYPE )
        MSG_WriteByte( msg, (int)ps->pmove.pm_type );
    ...
    //
    // write the rest of the player_state
    //
...
void SV_BuildClientFrame( client_t* client )
{
    ...
    for ( e = 1;
          e < quake2::getInstance()->quake2::getInstance()->ge.num_edicts; e++ )
    {
        ent = Edict_num( e );
...
void SV_RecordDemoMessage( void )
{
    ...
    // write a frame message that doesn't contain a player_state
    ...
    e = 1;
    ent = Edict_num( e );
    while ( e < quake2::getInstance()->quake2::getInstance()->ge.num_edicts )
    {
        // ignore ents without visible models unless they have an effect
