...
void SV_Begin_f( void )
{
    ...
    quake2::getInstance()->ge.ClientBegin( sv_player );
...
void SV_BeginDownload_f( void )
{
    ...
    if ( strstr( name, ".." ) ||
         !quake2::getInstance()->allow_download->value
         // leading dot is no good
         || *name == '.'
         // leading slash bad as well, must be in subdir
         || *name == '/'
         // next up, skin check
         || ( strncmp( name, "players/", 6 ) == 0 &&
              !quake2::getInstance()->allow_download_players->value )
         // now models
         ||
...
==================
*/
void SV_ShowServerinfo_f( void ) { Info_Print( Cvar_Serverinfo() ); }
...
void SV_Nextserver( void )
{
    ...
    if ( sv.state == ss_game ||
         ( sv.state == ss_pic && !Cvar_VariableValue( "coop" ) ) )
...
void SV_ExecuteUserCommand( char* s )
{
    ...
    if ( !u->name && sv.state == ss_game )
        quake2::getInstance()->ge.ClientCommand( sv_player );
...
void SV_ExecuteClientMessage( client_t* cl )
{
    ...
    usercmd_s nullcmd;
    usercmd_s oldest, oldcmd, newcmd;
    int net_drop;
    int stringCmdCount;
                ...
                if ( lastframe != cl->lastframe )
                {
                    ...
                    if ( cl->lastframe > 0 )
                    {
                        cl->frame_latency[cl->lastframe &
                                          ( LATENCY_COUNTS - 1 )] =
                            svs.realtime -
                            cl->frames[cl->lastframe & UPDATE_MASK].senttime;
                    }
                }
