...
void SVC_GetChallenge( void )
{
    ...
    // send it back
    Netchan_OutOfBandPrint( netsrc_t::NS_SERVER, net_from, "challenge %i",
...
gotnewcl:
    ...
    *newcl = temp;
    ...
    if ( !( quake2::getInstance()->quake2::getInstance()->ge.ClientConnect( ent, userinfo ) ) )
    {
...
{
    if ( rcon_password->string.empty() )
        return 0;

    if ( strcmp( Cmd_Argv( 1 ), rcon_password->string.c_str() ) )
        return 0;
...
void SV_PrepWorldFrame( void )
{
    ...
    for ( i = 0; i < quake2::getInstance()->quake2::getInstance()->ge.num_edicts; i++, ent++ )
    {
        ent = Edict_num( i );
...
void SV_RunGameFrame( void )
{
    if ( host_speeds->value )
        time_before_game = win32_Sys_Milliseconds();
    ...
    {
        quake2::getInstance()->quake2::getInstance()->ge.RunFrame();
    ...
    if ( host_speeds->value )
        time_after_game = win32_Sys_Milliseconds();
}
...
void Master_Heartbeat( void )
{
    ...
    if ( !quake2::getInstance()->dedicated->value )
        return;  // only quake2::getInstance()->dedicated servers send heartbeats

    if ( !public_server->value )
        return;  // a private quake2::getInstance()->dedicated game
    ...
    for ( i = 0; i < MAX_MASTERS; i++ )
        if ( master_adr[i].port )
        {
            ...
            Netchan_OutOfBandPrint( netsrc_t::NS_SERVER, master_adr[i],
                                    "heartbeat\n%s", string );
        }
}
...
void SV_UserinfoChanged( client_t* cl )
{
    ...
    strncpy( cl->name, Info_ValueForKey( cl->userinfo, "name" ),
             sizeof( cl->name ) - 1 );
    // mask off high bit
    for ( i = 0; i < sizeof( cl->name ); i++ ) cl->name[i] &= 127;
...
void SV_Init( void )
{
    ...
    quake2::getInstance()->allow_download =
        Cvar_Get( "allow_download", "0", CVAR_ARCHIVE );

    quake2::getInstance()->allow_download_players =
        Cvar_Get( "allow_download_players", "0", CVAR_ARCHIVE );
    quake2::getInstance()->allow_download_models =
        Cvar_Get( "allow_download_models", "1", CVAR_ARCHIVE );
    quake2::getInstance()->allow_download_sounds =
        Cvar_Get( "allow_download_sounds", "1", CVAR_ARCHIVE );
    quake2::getInstance()->allow_download_maps =
        Cvar_Get( "allow_download_maps", "1", CVAR_ARCHIVE );
