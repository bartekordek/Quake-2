...
void CMod_LoadLeafBrushes (lump_t *l)
{
	...
	unsigned short* in = (unsigned short*)(cmod_base + l->fileofs);
	if (l->filelen % sizeof(*in))
		Com_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size");
	count = l->filelen / sizeof(*in);
					  ...
					  trace_t *trace, cbrush_t *brush)
{
	...
	for (i=0 ; i<brush->numsides ; i++)
	{
		...
		if (d2 > 0)
			getout = e_true;	// endpoint is not in solid
		if (d1 > 0)
			startout = e_true;
	...
	{	// original point was inside brush
		trace->startsolid = e_true;
		if (!getout)
			trace->allsolid = e_true;
		return;
	}
					  ...
					  trace_t *trace, cbrush_t *brush)
{
	...
	// inside this brush
	trace->startsolid = trace->allsolid = e_true;
	trace->fraction = 0;
	trace->contents = brush->contents;
}
						  ...
						  int headnode, int brushmask)
{
	...
	{
		trace_ispoint = e_true;
		VectorClear (trace_extents);
	}
	else
	{
		trace_ispoint = e_false;
...
Returns e_true if any leaf under headnode has a cluster that
