...
/*
** VID_CreateWindow_win32
*/
#define WINDOW_CLASS_NAME "Quake 2"

void VID_CreateWindow_win32( int width, int height, int stylebits )
{
    WNDCLASS wc;
    RECT r;
    ...
    sww_state.hWnd =
        CreateWindowEx( exstyle, WINDOW_CLASS_NAME, "Quake 2", stylebits, x, y,
                        w, h, NULL, NULL, sww_state.hInstance, NULL );
...
static qboolean SWimp_InitGraphics( qboolean fullscreen )
{
    ...
    // create a new window
    VID_CreateWindow_win32( vid.width, vid.height, WINDOW_STYLE );
...
void SWimp_EndFrame( void )
{
    if( !sw_state.fullscreen )
    {
        ...
        BitBlt( sww_state.hDC, 0, 0, vid.width, vid.height,
                sww_state.hdcDIBSection, 0, 0, SRCCOPY );
    ...
    else
    {
        ...
        if( sww_state.modex )
        {
            if( ( rval = sww_state.lpddsBackBuffer->lpVtbl->BltFast(
                      sww_state.lpddsBackBuffer, 0, 0,
                      sww_state.lpddsOffScreenBuffer, &r, DDBLTFAST_WAIT ) ) ==
                DDERR_SURFACELOST )
            {
                sww_state.lpddsBackBuffer->lpVtbl->Restore(
                    sww_state.lpddsBackBuffer );
                sww_state.lpddsBackBuffer->lpVtbl->BltFast(
                    sww_state.lpddsBackBuffer, 0, 0,
                    sww_state.lpddsOffScreenBuffer, &r, DDBLTFAST_WAIT );
            }

            if( ( rval = sww_state.lpddsFrontBuffer->lpVtbl->Flip(
                      sww_state.lpddsFrontBuffer, NULL, DDFLIP_WAIT ) ) ==
                DDERR_SURFACELOST )
            {
                sww_state.lpddsFrontBuffer->lpVtbl->Restore(
                    sww_state.lpddsFrontBuffer );
                sww_state.lpddsFrontBuffer->lpVtbl->Flip(
                    sww_state.lpddsFrontBuffer, NULL, DDFLIP_WAIT );
            }
        }
        else
        {
            if( ( rval = sww_state.lpddsBackBuffer->lpVtbl->BltFast(
                      sww_state.lpddsFrontBuffer, 0, 0,
                      sww_state.lpddsOffScreenBuffer, &r, DDBLTFAST_WAIT ) ) ==
                DDERR_SURFACELOST )
            {
                sww_state.lpddsBackBuffer->lpVtbl->Restore(
        ...
        sww_state.lpddsOffScreenBuffer->lpVtbl->Lock(
            sww_state.lpddsOffScreenBuffer, NULL, &ddsd, DDLOCK_WAIT, NULL );
...
void SWimp_Shutdown( void )
{
    ...
    if( sww_state.hWnd )
    {
        ...
        ShowWindow(
            sww_state.hWnd,
            SW_SHOWNORMAL );  // prevents leaving empty slots in the taskbar
