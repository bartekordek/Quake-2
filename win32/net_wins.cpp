...
void NET_OpenIP (void)
{
    ...
    if (!ip_sockets[netsrc_t::NS_SERVER])
    {
        ...
        ip_sockets[netsrc_t::NS_SERVER] = NET_IPSocket (ip->string, port);
        if (!ip_sockets[netsrc_t::NS_SERVER] && dedicated)
    ...
    if (!ip_sockets[netsrc_t::NS_CLIENT])
    {
        ...
        ip_sockets[netsrc_t::NS_CLIENT] = NET_IPSocket (ip->string, port);
        if (!ip_sockets[netsrc_t::NS_CLIENT])
            ip_sockets[netsrc_t::NS_CLIENT] = NET_IPSocket (ip->string, PORT_ANY);
    }
}
...
void NET_OpenIPX (void)
{
    ...
    if (!ipx_sockets[netsrc_t::NS_SERVER])
    {
        ...
        ipx_sockets[netsrc_t::NS_SERVER] = NET_IPXSocket (port);
    }
    ...
    if (!ipx_sockets[netsrc_t::NS_CLIENT])
    {
        ...
        ipx_sockets[netsrc_t::NS_CLIENT] = NET_IPXSocket (port);
        if (!ipx_sockets[netsrc_t::NS_CLIENT])
            ipx_sockets[netsrc_t::NS_CLIENT] = NET_IPXSocket (PORT_ANY);
    }
}
...
void NET_Sleep(int msec)
{
    ...
    if (ip_sockets[netsrc_t::NS_SERVER]) {
        ...
        i = ip_sockets[netsrc_t::NS_SERVER];
    }
    if (ipx_sockets[netsrc_t::NS_SERVER]) {
        FD_SET(ipx_sockets[netsrc_t::NS_SERVER], &fdset); // network socket
        if (ipx_sockets[netsrc_t::NS_SERVER] > i)
            i = ipx_sockets[netsrc_t::NS_SERVER];
    }
