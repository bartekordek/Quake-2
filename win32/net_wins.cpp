...
char* NET_AdrToString( netadr_t a )
{
    ...
    else
    {
        ...
        // "%02x%02x%02x%02x:%02x%02x%02x%02x%02x%02x:%i", a.ipx[0], a.ipx[1],
        // a.ipx[2], a.ipx[3], a.ipx[4], a.ipx[5], a.ipx[6], a.ipx[7], a.ipx[8],
        // a.ipx[9], ntohs(a.port));
    }
...
bool NET_StringToSockaddr( char* s, struct sockaddr* sadr )
{
         ...
         ( s[21] == ':' ) )  // check for an IPX address
    {
        ...
        DO( 19, sa_nodenum[5] );
        sscanf( &s[22], "%u", &val );
        ( (struct sockaddr_ipx*)sadr )->sa_socket =
...
bool NET_GetLoopPacket( netsrc_t sock, netadr_t* net_from,
                        sizebuf_t* net_message )
{
    int i;
    loopback_t* loop;

    loop = &loopbacks[(size_t)sock];
...
bool NET_GetPacket( netsrc_t sock, netadr_t* net_from, sizebuf_t* net_message )
{
    ...
    {
        if ( protocol == 0 )
            net_socket = ip_sockets[(size_t)sock];
        else
            net_socket = ipx_sockets[(size_t)sock];
        ...
        if ( ret == -1 )
        {
            ...
            {
                // let quake2::getInstance()->dedicated servers continue after
                // errors Com_Printf_C ("NET_GetPacket: %s", NET_ErrorString());
            }
            else
            {
...
void NET_SendPacket( netsrc_t sock, int length, void* data, netadr_t to )
{
    ...
    ret = sendto( net_socket, (const char*)data, length, 0, &addr,
                  sizeof( addr ) );
    if ( ret == -1 )
    {
        ...
        if ( quake2::getInstance()
                 ->dedicated->value )  // let quake2::getInstance()->dedicated
                                       // servers continue after errors
        {
...
int NET_IPSocket( char* net_interface, int port )
{
    ...
    if ( ( newsocket = socket( PF_INET, SOCK_DGRAM, IPPROTO_UDP ) ) == -1 )
    {
        ...
        if ( err != WSAEAFNOSUPPORT )
            // Com_Printf_C ("WARNING: UDP_OpenSocket: socket: %s",
            // NET_ErrorString());
            return 0;
    }
    ...
    if ( setsockopt( newsocket, SOL_SOCKET, SO_BROADCAST, (char*)&i,
                     sizeof( i ) ) == -1 )
    {
        // Com_Printf_C ("WARNING: UDP_OpenSocket: setsockopt SO_BROADCAST:
        // %s\n", NET_ErrorString());
        return 0;
    }
...
void NET_OpenIP( void )
{
    ...
    cvar* ip = Cvar_Get( "ip", "localhost", CVAR_NOSET );

    quake2::getInstance()->dedicated->value =
        Cvar_VariableValue( "quake2::getInstance()->dedicated" );

    if ( !ip_sockets[(size_t)netsrc_t::NS_SERVER] )
    {
        ...
        ip_sockets[(size_t)netsrc_t::NS_SERVER] =
            NET_IPSocket( toChar( ip->string ), port );
        if ( !ip_sockets[(size_t)netsrc_t::NS_SERVER] &&
             quake2::getInstance()->dedicated )
            Com_Error( ERR_FATAL,
                       "Couldn't allocate quake2::getInstance()->dedicated "
                       "server IP port" );
    }

    // quake2::getInstance()->dedicated servers don't need client ports
    if ( quake2::getInstance()->dedicated )
        return;

    if ( !ip_sockets[(size_t)netsrc_t::NS_CLIENT] )
    {
        ...
        ip_sockets[(size_t)netsrc_t::NS_CLIENT] =
            NET_IPSocket( toChar( ip->string ), port );
        if ( !ip_sockets[(size_t)netsrc_t::NS_CLIENT] )
            ip_sockets[(size_t)netsrc_t::NS_CLIENT] =
                NET_IPSocket( toChar( ip->string ), PORT_ANY );
    }
}
...
void NET_OpenIPX( void )
{
    ...
    if ( !ipx_sockets[(size_t)netsrc_t::NS_SERVER] )
    {
        ...
        ipx_sockets[(size_t)netsrc_t::NS_SERVER] = NET_IPXSocket( port );
    }
    ...
    if ( !ipx_sockets[(size_t)netsrc_t::NS_CLIENT] )
    {
        ...
        ipx_sockets[(size_t)netsrc_t::NS_CLIENT] = NET_IPXSocket( port );
        if ( !ipx_sockets[(size_t)netsrc_t::NS_CLIENT] )
            ipx_sockets[(size_t)netsrc_t::NS_CLIENT] =
                NET_IPXSocket( PORT_ANY );
    }
}
...
void NET_Sleep( int msec )
{
    ...
    if ( ip_sockets[(size_t)netsrc_t::NS_SERVER] )
    {
        FD_SET( ip_sockets[(size_t)netsrc_t::NS_SERVER],
                &fdset );  // network socket
        i = ip_sockets[(size_t)netsrc_t::NS_SERVER];
    }
    if ( ipx_sockets[(size_t)netsrc_t::NS_SERVER] )
    {
        FD_SET( ipx_sockets[(size_t)netsrc_t::NS_SERVER],
                &fdset );  // network socket
        if ( ipx_sockets[(size_t)netsrc_t::NS_SERVER] > i )
            i = ipx_sockets[(size_t)netsrc_t::NS_SERVER];
    }
