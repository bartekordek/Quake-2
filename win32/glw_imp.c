...
qboolean VID_CreateWindow( int width, int height, qboolean fullscreen )
{
    ...
    glw_state.hWnd =
        CreateWindowEx( exstyle, WINDOW_CLASS_NAME, "Quake 2", stylebits, x, y,
                        w, h, NULL, NULL, glw_state.hInstance, NULL );
    ...
    if( fullscreen )
    {
        ...
        else
        {
            ...
            *pheight = height;
            ...
            ri.Con_Printf( PRINT_ALL,
                           "...calling CDS assuming dual monitors:" );
            ...
            /*
            ** our first CDS failed, so maybe we're running on some weird dual
            *monitor
            ** system
            */
            if( ChangeDisplaySettings( &dm, CDS_FULLSCREEN ) !=
...
void GLimp_Shutdown( void )
{
    if( qwglMakeCurrent && !qwglMakeCurrent( NULL, NULL ) )
        ri.Con_Printf( PRINT_ALL,
                       "ref_gl::R_Shutdown() - wglMakeCurrent failed\n" );
    if( glw_state.hGLRC )
    {
        if( qwglDeleteContext && !qwglDeleteContext( glw_state.hGLRC ) )
            ri.Con_Printf( PRINT_ALL,
                           "ref_gl::R_Shutdown() - wglDeleteContext failed\n" );
...
qboolean GLimp_InitGL( void )
{
    ...
    */
    ...
    {
        if( ( pixelformat = qwglChoosePixelFormat( glw_state.hDC, &pfd ) ) ==
            0 )
        {
            ri.Con_Printf( PRINT_ALL,
                           "GLimp_Init() - qwglChoosePixelFormat failed\n" );
            return false;
        }
        if( qwglSetPixelFormat( glw_state.hDC, pixelformat, &pfd ) == FALSE )
        {
            ri.Con_Printf( PRINT_ALL,
                           "GLimp_Init() - qwglSetPixelFormat failed\n" );
            return false;
        }
        qwglDescribePixelFormat( glw_state.hDC, pixelformat, sizeof( pfd ),
                                 &pfd );
    }
    else
    {
        ...
        {
            ri.Con_Printf( PRINT_ALL,
                           "GLimp_Init() - ChoosePixelFormat failed\n" );
            return false;
        }
        if( SetPixelFormat( glw_state.hDC, pixelformat, &pfd ) == FALSE )
        {
            ri.Con_Printf( PRINT_ALL,
                           "GLimp_Init() - SetPixelFormat failed\n" );
            return false;
        }
...
void GLimp_BeginFrame( float camera_separation )
{
    if( gl_bitdepth->modified )
    {
        if( gl_bitdepth->value != 0 && !glw_state.allowdisplaydepthchange )
        {
            ...
            ri.Con_Printf( PRINT_ALL,
                           "gl_bitdepth requires Win95 OSR2.x or WinNT 4.x\n" );
        }
