...
edict* SV_TestEntityPosition( edict* ent )
{
    ...
    trace = quake2::getInstance()->quake2::getInstance()->gi.trace(
        ent->s.origin, ent->mins, ent->maxs, ent->s.origin, ent, mask );
...
int SV_FlyMove( edict* ent, float time, int mask )
{
    ...
    for ( bumpcount = 0; bumpcount < numbumps; bumpcount++ )
    {
        ...
        trace = quake2::getInstance()->gi.trace( ent->s.origin, ent->mins,
                                                  ent->maxs, end, ent, mask );
        ...
        {  // go along the crease
            if ( numplanes != 2 )
            {
                //                quake2::getInstance()->gi.dprintf ("clip
...
retry:
    ...
    trace = quake2::getInstance()->gi.trace( start, ent->mins, ent->maxs, end,
                                              ent, mask );
...
bool SV_Push( edict* pusher, vec3_t move, vec3_t amove )
{
    ...
    for ( e = 1; e < globals.num_edicts; e++, check++ )
    {
        ...
        if ( check->groundentity != pusher )
        {
            ...
            if ( check->absmin[0] >= maxs[0] || check->absmin[1] >= maxs[1] ||
                 check->absmin[2] >= maxs[2] || check->absmax[0] <= mins[0] ||
                 check->absmax[1] <= mins[1] || check->absmax[2] <= mins[2] )
                continue;
    ...
    for ( p = pushed_p - 1; p >= pushed; p-- ) G_TouchTriggers( p->ent );
...
void SV_Physics_Pusher( edict* ent )
{
    ...
    if ( pushed_p > &pushed[MAX_EDICTS] )
        quake2::getInstance()->gi.error(
            ERR_FATAL, "pushed_p > &pushed[MAX_EDICTS], memory corrupted" );
...
void SV_Physics_Toss( edict* ent )
{
    ...
    if ( !wasinwater && isinwater )
        quake2::getInstance()->gi.positioned_sound(
            old_origin, g_edicts, CHAN_AUTO,
            quake2::getInstance()->gi.soundindex( "misc/h2ohit1.wav" ), 1, 1,
            0 );
    else if ( wasinwater && !isinwater )
        quake2::getInstance()->gi.positioned_sound(
            ent->s.origin, g_edicts, CHAN_AUTO,
            quake2::getInstance()->gi.soundindex( "misc/h2ohit1.wav" ), 1, 1,
            0 );
...
void G_RunEntity( edict* ent )
{
    ...
    switch ( (int)ent->movetype )
    {
        ...
        case MOVETYPE_FLYMISSILE:
            ...
            break;
        default:
            quake2::getInstance()->gi.error( "SV_Physics: bad movetype %i",
                                              (int)ent->movetype );
    }
}
