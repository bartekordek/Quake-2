...
#include <math.h>
float Q_ftol( float f ) { return fabs( f ); }
#endif
                              ...
                              float degrees )
{
    ...
    CrossProduct( vr, vf, quake2::getInstance()->vup );

    m[0][0] = vr[0];
    m[1][0] = vr[1];
    m[2][0] = vr[2];

    m[0][1] = quake2::getInstance()->vup[0];
    m[1][1] = quake2::getInstance()->vup[1];
    m[2][1] = quake2::getInstance()->vup[2];
...
char* COM_FileExtension( char* in )
{
    ...
    if ( !*in )
        return "";
    in++;
    for ( i = 0; i < 7 && *in; i++, in++ ) exten[i] = *in;
    exten[i] = 0;
    return exten;
}
...
void Com_PageInMemory( byte* buffer, int size )
{
    ...
    for ( i = size - 1; i > 0; i -= 4096 ) paged_total += buffer[i];
}
...
//     int        len;
//     va_list        argptr;
//     unsigned char    bigbuffer[0x10000];
...
void Info_SetValueForKey( const std::string& ss, const std::string& key,
