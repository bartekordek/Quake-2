...
void ai_stand( edict* self, float dist )
{
    ...
    if ( self->monsterinfoVal.aiflags & AI_STAND_GROUND )
    {
        if ( self->enemy )
        {
            ...
            if ( self->s.angles[YAW] != self->ideal_yaw &&
                 self->monsterinfoVal.aiflags & AI_TEMP_STAND_GROUND )
            {
                self->monsterinfoVal.aiflags &=
    ...
    if ( !( self->spawnflags & 1 ) && ( self->monsterinfoVal.idle ) &&
         ( level.time > self->monsterinfoVal.idle_time ) )
    {
...
bool visible( edict* self, edict* other )
{
    ...
    trace = quake2::getInstance()->quake2::getInstance()->gi.trace(
        spot1, vec3_origin, vec3_origin, spot2, self, MASK_OPAQUE );
...
void FoundTarget( edict* self )
{
    ...
    if ( !self->movetarget )
    {
        ...
        HuntTarget( self );
        quake2::getInstance()->quake2::getInstance()->gi.dprintf(
...
bool FindTarget( edict* self )
{
    ...
    else  // heardit
    {
        ...
        {
            if ( !quake2::getInstance()->gi.inPHS( self->s.origin,
                                                    client->s.origin ) )
                return false;
        }
        ...
        if ( client->areanum != self->areanum )
            if ( !quake2::getInstance()->gi.AreasConnected( self->areanum,
                                                             client->areanum ) )
                return false;
...
void ai_run( edict* self, float dist )
{
    ...
    if ( ( self->monsterinfoVal.search_time ) &&
         ( level.time > ( self->monsterinfoVal.search_time + 20 ) ) )
    {
        M_MoveToGoal( self, dist );
    ...
    if ( self->monsterinfoVal.aiflags & AI_PURSUE_NEXT )
    {
        ...
        if ( self->monsterinfoVal.aiflags & AI_PURSUE_TEMP )
        {
            ...
            marker = NULL;
            VectorCopy( self->monsterinfoVal.saved_goal,
                        self->monsterinfoVal.last_sighting );
            isNew = true;
        }
    ...
    if ( isNew )
    {
        //        quake2::getInstance()->gi.dprintf("checking for course
        //        correction\n");

        tr = quake2::getInstance()->gi.trace(
            self->s.origin, self->mins, self->maxs,
            self->monsterinfoVal.last_sighting, self, MASK_PLAYERSOLID );
        if ( tr.fraction < 1 )
        {
            ...
            G_ProjectSource( self->s.origin, v, v_forward, v_right,
                             left_target );
            tr = quake2::getInstance()->gi.trace( self->s.origin, self->mins,
                                                   self->maxs, left_target,
            ...
            if ( left >= center && left > right )
            {
                if ( left < 1 )
                {
                    ...
                    G_ProjectSource( self->s.origin, v, v_forward, v_right,
                                     left_target );
                    //                    quake2::getInstance()->gi.dprintf("incomplete
                    //                    path, go part way and adjust
                    //                    again\n");
                }
                VectorCopy( self->monsterinfoVal.last_sighting,
                            self->monsterinfoVal.saved_goal );
                self->monsterinfoVal.aiflags |= AI_PURSUE_TEMP;
                VectorCopy( left_target, self->goalentity->s.origin );
                VectorCopy( left_target, self->monsterinfoVal.last_sighting );
                VectorSubtract( self->goalentity->s.origin, self->s.origin, v );
                self->s.angles[YAW] = self->ideal_yaw = vectoyaw( v );
                //                quake2::getInstance()->gi.dprintf("adjusted
                //                left\n"); debug_drawline(self.origin,
                //                self.last_sighting, 152);
            }
