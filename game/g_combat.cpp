...
bool CanDamage( edict* targ, edict* inflictor )
{
    ...
    if ( targ->movetype == MOVETYPE_PUSH )
    {
        ...
        trace = quake2::getInstance()->quake2::getInstance()->gi.trace(
            inflictor->s.origin, vec3_origin, vec3_origin, dest, inflictor,
            MASK_SOLID );
    ...
    trace = quake2::getInstance()->quake2::getInstance()->gi.trace(
        inflictor->s.origin, vec3_origin, vec3_origin, targ->s.origin,
    ...
    dest[0] -= 15.0;
    dest[1] += 15.0;
    trace = quake2::getInstance()->gi.trace( inflictor->s.origin, vec3_origin,
                                              vec3_origin, dest, inflictor,
                                              MASK_SOLID );
    if ( trace.fraction == 1.0 )
        return true;
...
============
*/
void Killed( edict* targ, edict* inflictor, edict* attacker, int damage,
             vec3_t point )
{
    if ( targ->health < -999 )
        targ->health = -999;
...
============
*/
static int CheckPowerArmor( edict* ent, vec3_t point, vec3_t normal, int damage,
                            int dflags )
{
    gclient_t* client;
    int save;
...
static int CheckArmor( edict* ent, vec3_t point, vec3_t normal, int damage,
                       int te_sparks, int dflags )
{
    ...
    if ( dflags & DAMAGE_ENERGY )
        save =
            ceil( ( (gitem_armor_t*)armor->info )->energy_protection * damage );
    else
        save =
            ceil( ( (gitem_armor_t*)armor->info )->normal_protection * damage );
...
void M_ReactToDamage( edict* targ, edict* attacker )
{
    ...
    // it's not a tank (they spray too much), get mad at them
    ...
    if ( !( dflags & DAMAGE_NO_KNOCKBACK ) )
    {
             ...
             ( targ->movetype != MOVETYPE_STOP ) )
        {
            ...
            if ( targ->client && attacker == targ )
                VectorScale( dir, 1600.0 * (float)knockback / mass,
                             kvel );  // the rocket jump hack...
            else
