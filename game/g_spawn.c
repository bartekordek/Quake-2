...
===============
*/
void ED_ParseField (const char *key, char *value, edict_t *ent)
{
	field_t *f;
	byte	*b;
	float	 v;
...
void SpawnEntities (const char *mapname, char *entities, char *spawnpoint)
{
	...
	for (i = 0; i < game.maxclients; i++) g_edicts[i + 1].client = game.clients + i;
...
void SP_worldspawn (edict_t *ent)
{
	...
	ent->solid		  = SOLID_BSP;
	ent->inuse		  = e_true;	 // since the world doesn't use G_Spawn()
