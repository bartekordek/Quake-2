...
void Move_Final( edict* ent )
{
    ...
    VectorScale( ent->moveinfoVal.dir,
                 ent->moveinfoVal.remaining_distance / FRAMETIME, ent->velocity );
...
void Move_Begin( edict* ent )
{
    ...
    if ( ( ent->moveinfoVal.speed * FRAMETIME ) >=
         ent->moveinfoVal.remaining_distance )
    {
        Move_Final( ent );
        return;
    }
    VectorScale( ent->moveinfoVal.dir, ent->moveinfoVal.speed, ent->velocity );
    frames = floor( ( ent->moveinfoVal.remaining_distance / ent->moveinfoVal.speed ) /
                    FRAMETIME );
    ent->moveinfoVal.remaining_distance -=
        frames * ent->moveinfoVal.speed * FRAMETIME;
...
void Move_Calc( edict* ent, vec3_t dest, void ( *func )( edict* ) )
{
    ...
    if ( ent->moveinfoVal.speed == ent->moveinfoVal.accel &&
         ent->moveinfoVal.speed == ent->moveinfoVal.decel )
    {
...
void plat_CalcAcceleratedMove( moveinfo* moveinfo )
{
    ...
    if ( ( moveinfo->remaining_distance - accel_dist - decel_dist ) < 0 )
    {
        ...
        moveinfo->move_speed =
            ( -2 + sqrt( 4 - 4 * f * ( -2 * moveinfo->remaining_distance ) ) ) /
            ( 2 * f );
        decel_dist =
            AccelerationDistance( moveinfo->move_speed, moveinfo->decel );
    }
...
void plat_Accelerate( moveinfo* moveinfo )
{
    ...
    if ( moveinfo->current_speed == moveinfo->move_speed )
             ...
             moveinfo->decel_distance )
        {
            ...
            moveinfo->next_speed = moveinfo->move_speed -
    ...
    if ( moveinfo->current_speed < moveinfo->speed )
    {
        ...
        p2_distance =
            moveinfo->move_speed * ( 1.0 - ( p1_distance / p1_speed ) );
        distance = p1_distance + p2_distance;
        moveinfo->current_speed =
            ( p1_speed * ( p1_distance / distance ) ) +
            ( moveinfo->move_speed * ( p2_distance / distance ) );
        moveinfo->next_speed =
            moveinfo->move_speed - moveinfo->decel * ( p2_distance / distance );
        return;
    }
...
void Think_AccelMove( edict* ent )
{
    ...
    VectorScale( ent->moveinfoVal.dir, ent->moveinfoVal.current_speed * 10,
                 ent->velocity );
    ent->nextthink = level.time + FRAMETIME;
    ent->think = Think_AccelMove;
}
    ...
    if ( !( ent->flags & FL_TEAMSLAVE ) )
    {
        if ( ent->moveinfoVal.sound_end )
            quake2::getInstance()->quake2::getInstance()->gi.sound(
                ent, CHAN_NO_PHS_ADD + CHAN_VOICE, ent->moveinfoVal.sound_end, 1,
                ATTN_STATIC, 0 );
        ent->s.sound = 0;
    }
...
void plat_go_down( edict* ent )
{
    if ( !( ent->flags & FL_TEAMSLAVE ) )
    {
        if ( ent->moveinfoVal.sound_start )
            quake2::getInstance()->gi.sound( ent, CHAN_NO_PHS_ADD + CHAN_VOICE,
                                              ent->moveinfoVal.sound_start, 1,
                                              ATTN_STATIC, 0 );
...
void Touch_Plat_Center( edict* ent, edict* other, plane_s* plane,
                        csurface_s* surf )
{
    if ( !other->client )
        return;
    ...
    if ( ent->moveinfoVal.state == STATE_BOTTOM )
        ...
        ent->nextthink =
            level.time +
            1;  // the player is still on the plat, so delay going down
}
...
disabled in the extended position until it is trigger, when it will lower and
...
void SP_func_plat( edict* ent )
{
    ...
    ent->moveinfoVal.sound_start =
        quake2::getInstance()->gi.soundindex( "plats/pt1_strt.wav" );
    ent->moveinfoVal.sound_middle =
        quake2::getInstance()->gi.soundindex( "plats/pt1_mid.wav" );
    ent->moveinfoVal.sound_end =
        quake2::getInstance()->gi.soundindex( "plats/pt1_end.wav" );
}

//====================================================================

/*QUAKED func_rotating (0 .5 .8) ? START_ON REVERSE X_AXIS Y_AXIS TOUCH_PAIN
STOP ANIMATED ANIMATED_FAST You need to have an origin brush as part of this
entity.  The center of that brush will be the point around which it is rotated.
It will rotate around the Z axis by default.  You can check either the X_AXIS or
Y_AXIS box to change that.
...
void rotating_touch( edict* self, edict* other, plane_s* plane,
                     csurface_s* surf )
{
...
void button_fire( edict* self )
{
    ...
    if ( self->moveinfoVal.sound_start && !( self->flags & FL_TEAMSLAVE ) )
        quake2::getInstance()->gi.sound( self, CHAN_NO_PHS_ADD + CHAN_VOICE,
                                          self->moveinfoVal.sound_start, 1,
                                          ATTN_STATIC, 0 );
...
void button_killed( edict* self, edict* inflictor, edict* attacker, int damage,
                    vec3_t point )
{
...
void SP_func_button( edict* ent )
{
    ...
    if ( ent->sounds != 1 )
        ent->moveinfoVal.sound_start =
            quake2::getInstance()->gi.soundindex( "switches/butn2.wav" );
...
/*QUAKED func_door (0 .5 .8) ? START_OPEN x CRUSHER NOMONSTER ANIMATED TOGGLE
ANIMATED_FAST TOGGLE        wait in both the start and end states for a trigger
event. START_OPEN    the door to moves to its destination when spawned, and
operate in reverse.  It is used to temporarily or permanently close off an area
when triggered (not useful for touch or takedamage doors). NOMONSTER    monsters
will not trigger this door

"message"    is printed when the door is touched if it is a trigger door and it
hasn't been fired yet "angle"        determines the opening direction
"targetname" if set, no touch field will be spawned and a remote button or
trigger field activates the door. "health"    if set, door must be shot open
    ...
    if ( !( self->flags & FL_TEAMSLAVE ) )
    {
        if ( self->moveinfoVal.sound_end )
            quake2::getInstance()->gi.sound(
                self, CHAN_NO_PHS_ADD + CHAN_VOICE, self->moveinfoVal.sound_end, 1,
                ATTN_STATIC, 0 );
        self->s.sound = 0;
    }
    self->moveinfoVal.state = STATE_TOP;
...
void door_go_down( edict* self )
{
    if ( !( self->flags & FL_TEAMSLAVE ) )
    {
        if ( self->moveinfoVal.sound_start )
            quake2::getInstance()->gi.sound(
                self, CHAN_NO_PHS_ADD + CHAN_VOICE, self->moveinfoVal.sound_start,
                1, ATTN_STATIC, 0 );
        self->s.sound = self->moveinfoVal.sound_middle;
    }
    if ( self->max_health )
...
void Touch_DoorTrigger( edict* self, edict* other, plane_s* plane,
                        csurface_s* surf )
{
    if ( other->health <= 0 )
        return;
...
void door_killed( edict* self, edict* inflictor, edict* attacker, int damage,
                  vec3_t point )
{
    edict* ent;
...
void door_touch( edict* self, edict* other, plane_s* plane, csurface_s* surf )
{
    ...
    quake2::getInstance()->gi.sound(
        other, CHAN_AUTO,
        quake2::getInstance()->gi.soundindex( "misc/talk1.wav" ), 1, ATTN_NORM,
        0 );
}
...
void SP_func_door( edict* ent )
{
    ...
    {
        ent->moveinfoVal.sound_start =
            quake2::getInstance()->gi.soundindex( "doors/dr1_strt.wav" );
        ent->moveinfoVal.sound_middle =
            quake2::getInstance()->gi.soundindex( "doors/dr1_mid.wav" );
        ent->moveinfoVal.sound_end =
            quake2::getInstance()->gi.soundindex( "doors/dr1_end.wav" );
    }
    ...
    abs_movedir[1] = fabs( ent->movedir[1] );
    abs_movedir[2] = fabs( ent->movedir[2] );
    ent->moveinfoVal.distance = abs_movedir[0] * ent->size[0] +
                             abs_movedir[1] * ent->size[1] +
                             abs_movedir[2] * ent->size[2] - st.lip;
...
/*QUAKED func_door_rotating (0 .5 .8) ? START_OPEN REVERSE CRUSHER NOMONSTER
ANIMATED TOGGLE X_AXIS Y_AXIS TOGGLE causes the door to wait in both the start
and end states for a trigger event.
...
triggered (not useful for touch or takedamage doors). NOMONSTER    monsters will
not trigger this door

You need to have an origin brush as part of this entity.  The center of that
brush will be the point around which it is rotated. It will rotate around the Z
axis by default.  You can check either the X_AXIS or Y_AXIS box to change that.
...
func_water is a moveable water brush.  It must be targeted to operate.  Use a
non-water texture at your own risk.

START_OPEN causes the water to move to its destination when spawned and operate
in reverse.
...
void SP_func_water( edict* self )
{
    ...
    switch ( self->sounds )
    {
        ...
        case 1:  // water
            self->moveinfoVal.sound_start =
                quake2::getInstance()->gi.soundindex( "world/mov_watr.wav" );
            self->moveinfoVal.sound_end =
                quake2::getInstance()->gi.soundindex( "world/stp_watr.wav" );
            break;
    ...
    self->moveinfoVal.distance = abs_movedir[0] * self->size[0] +
                              abs_movedir[1] * self->size[1] +
                              abs_movedir[2] * self->size[2] - st.lip;
...
until activated. speed    default 100 dmg        default    2 noise    looping
sound to play when the train is in motion
...
void train_wait( edict* self )
{
    ...
    if ( self->moveinfoVal.wait )
    {
        ...
        if ( !( self->flags & FL_TEAMSLAVE ) )
        {
            if ( self->moveinfoVal.sound_end )
                quake2::getInstance()->gi.sound(
                    self, CHAN_NO_PHS_ADD + CHAN_VOICE,
                    self->moveinfoVal.sound_end, 1, ATTN_STATIC, 0 );
            self->s.sound = 0;
        }
    }
    else
...
again:
    if ( !self->target )
    {
        //        quake2::getInstance()->gi.dprintf ("train_next: no next
        //        target\n");
        return;
    }
...
void SP_func_train( edict* self )
{
    ...
    if ( st.noise )
        self->moveinfoVal.sound_middle =
            quake2::getInstance()->gi.soundindex( st.noise );
...
void door_secret_die( edict* self, edict* inflictor, edict* attacker,
...
void SP_func_door_secret( edict* ent )
{
    ...
    ent->moveinfoVal.accel = ent->moveinfoVal.decel = ent->moveinfoVal.speed = 50;
