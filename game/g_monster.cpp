...
void M_MoveFrame( edict* self )
{
    ...
    if ( ( self->monsterinfoVal.nextframe ) &&
         ( self->monsterinfoVal.nextframe >= move->firstframe ) &&
         ( self->monsterinfoVal.nextframe <= move->lastframe ) )
    {
    ...
    if ( move->frame[index].aifunc )
        if ( !( self->monsterinfoVal.aiflags & AI_HOLD_FRAME ) )
            move->frame[index].aifunc(
                self, move->frame[index].dist * self->monsterinfoVal.scale );
        else
...
void monster_use( edict* self, edict* other, edict* activator )
{
    ...
    if ( activator->flags & FL_NOTARGET )
        return;
    if ( !( activator->client ) &&
         !( activator->monsterinfoVal.aiflags & AI_GOOD_GUY ) )
        return;
...
bool monster_start( edict* self )
{
    ...
    if ( self->monsterinfoVal.currentmove )
        self->s.frame =
            self->monsterinfoVal.currentmove->firstframe +
            ( rand() % ( self->monsterinfoVal.currentmove->lastframe -
                         self->monsterinfoVal.currentmove->firstframe + 1 ) );
