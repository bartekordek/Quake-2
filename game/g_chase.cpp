...
void UpdateChaseCam( edict* ent )
{
    ...
    trace = quake2::getInstance()->quake2::getInstance()->gi.trace(
        ownerv, vec3_origin, vec3_origin, o, targ, MASK_SOLID );
    ...
    o[2] += 6;
    trace = quake2::getInstance()->quake2::getInstance()->gi.trace(
        goal, vec3_origin, vec3_origin, o, targ, MASK_SOLID );
    if ( trace.fraction < 1 )
    {
        VectorCopy( trace.endpos, goal );
        goal[2] -= 6;
...
void GetChaseTarget( edict* ent )
{
    ...
    quake2::getInstance()->quake2::getInstance()->gi.centerprintf(
        ent, "No other players to chase." );
}
