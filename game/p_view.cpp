...
void SV_CalcViewOffset( edict* ent )
{
    ...
    if ( bob > 6 )
        bob = 6;
    // quake2::getInstance()->quake2::getInstance()->gi.DebugGraph (bob *2,
    // 255);
    v[2] += bob;
...
void SV_CalcBlend( edict* ent )
{
    ...
    contents = quake2::getInstance()->quake2::getInstance()->gi.pointcontents(
        vieworg );
...
void P_WorldEffects( void )
{
    ...
    if ( waterlevel == 3 )
    {
        ...
        if ( breather || envirosuit )
        {
            ...
            if ( ( (int)( current_client->breather_framenum - level.framenum ) %
                   25 ) == 0 )
            {
         ...
         ( current_player->watertype & ( CONTENTS_LAVA | CONTENTS_SLIME ) ) )
    {
        if ( current_player->watertype & CONTENTS_LAVA )
        {
            ...
            if ( envirosuit )  // take 1/3 damage with envirosuit
                T_Damage( current_player, world, world, vec3_origin,
                          current_player->s.origin, vec3_origin, 1 * waterlevel,
                          0, 0, MOD_LAVA );
            else
                T_Damage( current_player, world, world, vec3_origin,
                          current_player->s.origin, vec3_origin, 3 * waterlevel,
                          0, 0, MOD_LAVA );
        }
...
void G_SetClientSound( edict* ent )
{
    ...
    if ( ent->client->pers.helpchanged && ent->client->pers.helpchanged <= 3 &&
         !( level.framenum & 63 ) )
    {
...
void ClientEndServerFrame( edict* ent )
{
    ...
    if ( ent->client->showscores && !( level.framenum & 31 ) )
    {
        ...
        quake2::getInstance()->quake2::getInstance()->gi.unicast( ent,
                                                                    false );
    }
}
