...
static void SP_FixCoopSpots( edict* self )
{
    ...
    while ( 1 )
    {
        ...
        if ( VectorLength( d ) < 384 )
        {
            ...
            {
                //                quake2::getInstance()->gi.dprintf("FixCoopSpots changed %s at %s
                //                targetname from %s to %s\n", self->classname,
...
void ClientObituary( edict* self, edict* inflictor, edict* attacker )
{
    ...
    if ( deathmatch->value || coop->value )
    {
        ...
        if ( message )
        {
            quake2::getInstance()->gi.bprintf( PRINT_MEDIUM, "%s %s.\n", self->client->pers.netname,
                        message );
            if ( deathmatch->value )
        ...
        if ( attacker && attacker->client )
        {
            ...
            if ( message )
            {
                quake2::getInstance()->gi.bprintf( PRINT_MEDIUM, "%s %s %s%s\n",
    ...
    quake2::getInstance()->gi.bprintf( PRINT_MEDIUM, "%s died.\n", self->client->pers.netname );
    if ( deathmatch->value )
        self->client->resp.score--;
}
...
void TossClientWeapon( edict* self )
{
    ...
    if ( quad )
    {
        ...
        drop->nextthink =
            level.time +
            ( self->client->quad_framenum - level.framenum ) * FRAMETIME;
...
==================
*/
void player_die( edict* self, edict* inflictor, edict* attacker, int damage,
                 vec3_t point )
{
    int n;
    ...
    if ( self->health < -40 )
    {  // gib
        quake2::getInstance()->gi.sound( self, CHAN_BODY, quake2::getInstance()->gi.soundindex( "misc/udeath.wav" ), 1,
                  ATTN_NORM, 0 );
        for ( n = 0; n < 4; n++ )
    ...
    {  // normal death
        if ( !self->deadflag )
        {
            ...
            quake2::getInstance()->gi.sound( self, CHAN_VOICE,
                      quake2::getInstance()->gi.soundindex( va( "*death%i.wav", ( rand() % 4 ) + 1 ) ),
                      1, ATTN_NORM, 0 );
        }
    }
...
void SelectSpawnPoint( edict* ent, vec3_t origin, vec3_t angles )
{
    ...
    if ( !spot )
    {
        ...
        if ( !spot )
        {
            ...
            if ( !spot )
                quake2::getInstance()->gi.error( "Couldn't find spawn point %s\n", game.spawnpoint );
        }
    }
...
void body_die( edict* self, edict* inflictor, edict* attacker, int damage,
               vec3_t point )
{
    int n;
...
void CopyToBodyQue( edict* ent )
{
    ...
    quake2::getInstance()->gi.unlinkentity( body );
    body->s = ent->s;
    ...
    quake2::getInstance()->gi.linkentity( body );
}
...
void respawn( edict* self )
{
    ...
    quake2::getInstance()->gi.AddCommandString( "menu_loadgame\n" );
}

bool isPasswordEqual( const std::string& password, const std::string& value )
{
    if ( password.empty() )
    {
        return false;
 ...
 */
void spectator_respawn( edict* ent )
{
    int i, numspec;
    ...
    {
        char* value =
            Info_ValueForKey( ent->client->pers.userinfo, "spectator" );
        if ( !spectator_password->string.empty() &&
             spectator_password->string == "none" &&
             spectator_password->string == value )
        {
            quake2::getInstance()->gi.cprintf( ent, PRINT_HIGH, "Spectator password incorrect.\n" );
            ent->client->pers.spectator = false;
            quake2::getInstance()->gi.WriteByte( svc_stufftext );
            quake2::getInstance()->gi.WriteString( "spectator 0\n" );
        ...
        {
            quake2::getInstance()->gi.cprintf( ent, PRINT_HIGH, "Server spectator limit is full." );
            ent->client->pers.spectator = false;
    ...
    else
    {
        ...
        char* value =
            Info_ValueForKey( ent->client->pers.userinfo, "password" );
        if ( isPasswordEqual( password->string, value ) )
        {
            quake2::getInstance()->gi.cprintf( ent, PRINT_HIGH, "Password incorrect.\n" );
            ent->client->pers.spectator = true;
            quake2::getInstance()->gi.WriteByte( svc_stufftext );
            quake2::getInstance()->gi.WriteString( "spectator 1\n" );
            quake2::getInstance()->gi.unicast( ent, true );
            return;
        }
    }
    ...
    if ( !ent->client->pers.spectator )
    {
        ...
        quake2::getInstance()->gi.WriteByte( MZ_LOGIN );
    ...
    if ( ent->client->pers.spectator )
        quake2::getInstance()->gi.bprintf( PRINT_HIGH, "%s has moved to the sidelines\n",
                    ent->client->pers.netname );
    else
        quake2::getInstance()->gi.bprintf( PRINT_HIGH, "%s joined the game\n",
                    ent->client->pers.netname );
}
...
void PutClientInServer( edict* ent )
{
    ...
    else if ( coop->value )
    {
        ...
        //                resp.coop_respawn.inventory[n] =
        //                client->pers.inventory[n];
        //        }
    ...
    {
        client->ps.fov =
            atoi( Info_ValueForKey( client->pers.userinfo, "fov" ) );
        if ( client->ps.fov < 1 )
            client->ps.fov = 90;
    ...
    client->ps.gunindex = quake2::getInstance()->gi.modelindex( client->pers.weapon->view_model );
...
void ClientBeginDeathmatch( edict* ent )
{
    ...
    quake2::getInstance()->gi.bprintf( PRINT_HIGH, "%s entered the game\n",
                ent->client->pers.netname );
...
void ClientUserinfoChanged( edict* ent, char* userinfo )
{
    ...
    strncpy( ent->client->pers.netname, s,
    ...
    quake2::getInstance()->gi.configstring( CS_PLAYERSKINS + playernum,
...
bool ClientConnect( edict* ent, char* userinfo )
{
    ...
    if ( deathmatch->value && *value && strcmp( value, "0" ) )
    {
        ...
        if ( isPasswordEqual( spectator_password->string, value ) )
        {
            // Info_SetValueForKey(userinfo, "rejmsg", "Spectator password
        ...
        for ( i = numspec = 0; i < maxclients->value; i++ )
            if ( g_edicts[i + 1].inuse &&
                 g_edicts[i + 1].client->pers.spectator )
                numspec++;

        if ( numspec >= maxspectators->value )
        {
            // Info_SetValueForKey(userinfo, "rejmsg", "Server spectator limit
            // is full.");
            return false;
        }
    }
    ...
    if ( game.maxclients > 1 )
        quake2::getInstance()->gi.dprintf( "%s connected\n", ent->client->pers.netname );
...
void ClientDisconnect( edict* ent )
{
    ...
    quake2::getInstance()->gi.bprintf( PRINT_HIGH, "%s disconnected\n", ent->client->pers.netname );

    // send effect
    quake2::getInstance()->gi.WriteByte( svc_muzzleflash );
    quake2::getInstance()->gi.WriteShort( ent - g_edicts );
    quake2::getInstance()->gi.WriteByte( MZ_LOGOUT );
    quake2::getInstance()->gi.multicast( ent->s.origin, multicast_t::MULTICAST_PVS );
...
trace_t PM_trace( vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end )
{
    if ( pm_passent->health > 0 )
        return quake2::getInstance()->gi.trace( start, mins, maxs, end, pm_passent, MASK_PLAYERSOLID );
    else
        return quake2::getInstance()->gi.trace( start, mins, maxs, end, pm_passent, MASK_DEADSOLID );
}
...
==============
*/
void ClientThink( edict* ent, usercmd_s* ucmd )
{
    gclient_t* client;
    edict* other;
    int i, j;
    ...
    else
    {
        ...
        {
            pm.snapinitial = true;
            //        quake2::getInstance()->gi.dprintf ("pmove changed!\n");
        }

        pm.cmd = *ucmd;

        pm.trace = PM_trace;  // adds default parms
        pm.pointcontents = quake2::getInstance()->gi.pointcontents;

        // perform a pmove
        quake2::getInstance()->gi.Pmove( &pm );
        ...
        if ( ent->groundentity && !pm.groundentity && ( pm.cmd.upmove >= 10 ) &&
             ( pm.waterlevel == 0 ) )
        {
            quake2::getInstance()->gi.sound( ent, CHAN_VOICE, quake2::getInstance()->gi.soundindex( "*jump1.wav" ), 1,
                      ATTN_NORM, 0 );
