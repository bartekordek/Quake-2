...
#include "shared/enum_multicast.hpp"
#include "shared/p_move.hpp"
...
#define SVF_DEADMONSTER \
    0x00000002                  // treat as CONTENTS_DEADMONSTER for collision
...
struct game_import
{
    ...
    void ( *sound )( edict* ent, int channel, int soundindex, float volume,
                     float attenuation, float timeofs );
    void ( *positioned_sound )( vec3_t origin, edict* ent, int channel,
    ...
    void ( *unlinkentity )(
        edict* ent );  // call before removing an interactive edict
    int ( *BoxEdicts )( vec3_t mins, vec3_t maxs, edict** list, int maxcount,
                        int areatype );
    void ( *Pmove )(
        pmove_t* pmove );  // player movement code common with client prediction
...
struct game_export_t
{
    ...
    void ( *ClientThink )( edict* ent, usercmd_s* cmd );

    void ( *RunFrame )( void );

    // ServerCommand will be called when an "sv <command>" command is issued on
    // the server console. The game can issue quake2::getInstance()->gi.argc()
    // / quake2::getInstance()->gi.argv() commands to get the rest of the
    // parameters
    ...
    //
    // The size will be fixed when quake2::getInstance()->ge.Init() is called
    edict* edicts;
    int edict_size;
