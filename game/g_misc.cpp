...
void Use_Areaportal( edict* ent, edict* other, edict* activator )
{
    ...
    //    ent->style, ent->count);
    quake2::getInstance()->quake2::getInstance()->gi.SetAreaPortalState(
        ent->style, ent->count );
}
...
void ClipGibVelocity( edict* ent )
{
...
void gib_touch( edict* self, edict* other, plane_s* plane, csurface_s* surf )
{
    ...
    if ( plane )
    {
        quake2::getInstance()->quake2::getInstance()->gi.sound(
            self, CHAN_VOICE,
            quake2::getInstance()->gi.soundindex( "misc/fhit3.wav" ), 1,
            ATTN_NORM, 0 );
...
void gib_die( edict* self, edict* inflictor, edict* attacker, int damage,
              vec3_t point )
{
    G_FreeEdict( self );
}
...
=================
*/
void debris_die( edict* self, edict* inflictor, edict* attacker, int damage,
                 vec3_t point )
{
    G_FreeEdict( self );
}
...
void path_corner_touch( edict* self, edict* other, plane_s* plane,
                        csurface_s* surf )
{
    vec3_t v;
    edict* next;
...
*/
void point_combat_touch( edict* self, edict* other, plane_s* plane,
                         csurface_s* surf )
{
    edict* activator;
...
void SP_viewthing( edict* ent )
{
    ...
    ent->s.modelindex = quake2::getInstance()->gi.modelindex(
...
*/
void SP_info_null( edict* self ) { G_FreeEdict( self ); };
...
static void light_use( edict* self, edict* other, edict* activator )
{
...
void SP_light( edict* self )
{
    ...
    if ( self->style >= 32 )
    {
        self->use = light_use;
        if ( self->spawnflags & START_OFF )
            quake2::getInstance()->gi.configstring( CS_LIGHTS + self->style,
                                                     "a" );
        else
            quake2::getInstance()->gi.configstring( CS_LIGHTS + self->style,
                                                     "m" );
...
void func_wall_use( edict* self, edict* other, edict* activator )
{
...
void SP_func_wall( edict* self )
{
    ...
    {
        //        quake2::getInstance()->gi.dprintf("func_wall missing
        //        TRIGGER_SPAWN\n");
        self->spawnflags |= 1;
    }
...
void func_object_touch( edict* self, edict* other, plane_s* plane,
                        csurface_s* surf )
{
...
void func_object_release( edict* self )
{
    self->movetype = MOVETYPE_TOSS;
    self->touch = func_object_touch;
}

void func_object_use( edict* self, edict* other, edict* activator )
{
    self->solid = SOLID_BSP;
    self->svflags &= ~SVF_NOCLIENT;
    self->use = NULL;
...
*/
void func_explosive_explode( edict* self, edict* inflictor, edict* attacker,
...
void func_explosive_use( edict* self, edict* other, edict* activator )
{
    func_explosive_explode( self, self, other, self->health, vec3_origin );
}

void func_explosive_spawn( edict* self, edict* other, edict* activator )
{
    self->solid = SOLID_BSP;
    self->svflags &= ~SVF_NOCLIENT;
    self->use = NULL;
...
void barrel_explode( edict* self )
{
    vec3_t org;
    float spd;
    vec3_t save;
...
void barrel_delay( edict* self, edict* inflictor, edict* attacker, int damage,
                   vec3_t point )
{
    self->takedamage = DAMAGE_NO;
    self->nextthink = level.time + 2 * FRAMETIME;
...
void misc_blackhole_use( edict* ent, edict* other, edict* activator )
{
    ...
    quake2::getInstance()->gi.multicast (ent->s.origin,
    multicast_t::MULTICAST_PVS);
*/
    G_FreeEdict( ent );
}

void misc_blackhole_think( edict* self )
{
    if ( ++self->s.frame < 19 )
        self->nextthink = level.time + FRAMETIME;
    else
...
void SP_misc_blackhole( edict* ent )
{
    ent->movetype = MOVETYPE_NONE;
    ent->solid = SOLID_NOT;
...
void misc_eastertank_think( edict* self )
{
    if ( ++self->s.frame < 293 )
        self->nextthink = level.time + FRAMETIME;
    else
...
void SP_misc_eastertank( edict* ent )
{
    ent->movetype = MOVETYPE_NONE;
    ent->solid = SOLID_BBOX;
...
void misc_easterchick_think( edict* self )
{
    if ( ++self->s.frame < 247 )
        self->nextthink = level.time + FRAMETIME;
    else
...
void SP_misc_easterchick( edict* ent )
{
    ent->movetype = MOVETYPE_NONE;
    ent->solid = SOLID_BBOX;
...
void misc_easterchick2_think( edict* self )
{
    if ( ++self->s.frame < 287 )
        self->nextthink = level.time + FRAMETIME;
    else
...
void SP_misc_easterchick2( edict* ent )
{
    ent->movetype = MOVETYPE_NONE;
    ent->solid = SOLID_BBOX;
    VectorSet( ent->mins, -32, -32, 0 );
...
void commander_body_think( edict* self )
{
    if ( ++self->s.frame < 24 )
        self->nextthink = level.time + FRAMETIME;
    else
    ...
    if ( self->s.frame == 22 )
        quake2::getInstance()->gi.sound(
            self, CHAN_BODY,
            quake2::getInstance()->gi.soundindex( "tank/thud.wav" ), 1,
            ATTN_NORM, 0 );
}
...
void commander_body_use( edict* self, edict* other, edict* activator )
{
    ...
    quake2::getInstance()->gi.sound(
        self, CHAN_BODY,
        quake2::getInstance()->gi.soundindex( "tank/pain.wav" ), 1, ATTN_NORM,
        0 );
}
...
*/
void misc_banner_think( edict* ent )
{
    ent->s.frame = ( ent->s.frame + 1 ) % 16;
    ent->nextthink = level.time + FRAMETIME;
}

void SP_misc_banner( edict* ent )
{
    ent->movetype = MOVETYPE_NONE;
    ent->solid = SOLID_NOT;
    ent->s.modelindex = quake2::getInstance()->gi.modelindex(
...
/*QUAKED misc_deadsoldier (1 .5 0) (-16 -16 0) (16 16 16) ON_BACK ON_STOMACH
BACK_DECAP FETAL_POS SIT_DECAP IMPALED This is the dead player model. Comes in 6
exciting different poses!
*/
void misc_deadsoldier_die( edict* self, edict* inflictor, edict* attacker,
                           int damage, vec3_t point )
{
    int n;
    ...
    quake2::getInstance()->gi.sound(
        self, CHAN_BODY,
        quake2::getInstance()->gi.soundindex( "misc/udeath.wav" ), 1,
        ATTN_NORM, 0 );
    for ( n = 0; n < 4; n++ )
...
void SP_misc_deadsoldier( edict* ent )
{
    if ( deathmatch->value )
    {  // auto-remove for deathmatch
        G_FreeEdict( ent );
...
extern void train_use( edict* self, edict* other, edict* activator );
extern void func_train_find( edict* self );

void misc_viper_use( edict* self, edict* other, edict* activator )
{
    self->svflags &= ~SVF_NOCLIENT;
    self->use = train_use;
...
void SP_misc_viper( edict* ent )
{
    ...
    ent->solid = SOLID_NOT;
    ent->s.modelindex =
        quake2::getInstance()->gi.modelindex( "models/ships/viper/tris.md2" );
    VectorSet( ent->mins, -16, -16, 0 );
    VectorSet( ent->maxs, 16, 16, 32 );
...
*/
void SP_misc_bigviper( edict* ent )
{
    ent->movetype = MOVETYPE_NONE;
    ent->solid = SOLID_BBOX;
...
*/
void misc_viper_bomb_touch( edict* self, edict* other, plane_s* plane,
                            csurface_s* surf )
{
...
void misc_viper_bomb_prethink( edict* self )
{
    vec3_t v;
    float diff;
...
void misc_viper_bomb_use( edict* self, edict* other, edict* activator )
{
    edict* viper;
...
void SP_misc_viper_bomb( edict* self )
{
    ...
    self->s.modelindex =
        quake2::getInstance()->gi.modelindex( "models/objects/bomb/tris.md2" );
...
void misc_strogg_ship_use( edict* self, edict* other, edict* activator )
{
    self->svflags &= ~SVF_NOCLIENT;
    self->use = train_use;
    train_use( self, other, activator );
...
void SP_misc_strogg_ship( edict* ent )
{
    if ( !ent->target )
    {
 ...
 */
void misc_satellite_dish_think( edict* self )
{
    self->s.frame++;
    if ( self->s.frame < 38 )
        self->nextthink = level.time + FRAMETIME;
...
void misc_satellite_dish_use( edict* self, edict* other, edict* activator )
{
    self->s.frame = 0;
...
void SP_misc_satellite_dish( edict* ent )
{
    ent->movetype = MOVETYPE_NONE;
    ent->solid = SOLID_BBOX;
    VectorSet( ent->mins, -64, -64, 0 );
...
{
    quake2::getInstance()->gi.setmodel( ent,
...
void target_string_use( edict* self, edict* other, edict* activator )
{
    edict* e;
    int n, l;
    char c;
...
void SP_target_string( edict* self )
{
    if ( !self->message )
        self->message = "";
...
/*QUAKED func_clock (0 0 1) (-8 -8 -8) (8 8 8) TIMER_UP TIMER_DOWN START_OFF
MULTI_USE target a target_string with this
...
static void func_clock_reset( edict* self )
{
    self->activator = NULL;
    if ( self->spawnflags & 1 )
    {
...
static void func_clock_format_countdown( edict* self )
{
    if ( self->style == 0 )
    {
...
void func_clock_think( edict* self )
{
    if ( !self->enemy )
    {
...
void func_clock_use( edict* self, edict* other, edict* activator )
{
    if ( !( self->spawnflags & 8 ) )
        self->use = NULL;
    if ( self->activator )
...
void SP_func_clock( edict* self )
{
    ...
    self->message = (char*)quake2::getInstance()->gi.TagMalloc(
...
void teleporter_touch( edict* self, edict* other, plane_s* plane,
                       csurface_s* surf )
{
    edict* dest;
    int i;
...
*/
void SP_misc_teleporter( edict* ent )
{
    edict* trig;
