...
typedef struct gitem
{
    ...
    char* precaches;  // string of all models, sounds, and images this item will
                      // use
} gitem;
...
// ZOID
extern cvar* capturelimit;
extern cvar* instantweap;
// ZOID
...
typedef enum
{
    fieldtype_t::F_INT,
    fieldtype_t::F_FLOAT,
    fieldtype_t::F_LSTRING,  // string on disk, pointer in memory, TAG_LEVEL
    fieldtype_t::F_GSTRING,  // string on disk, pointer in memory, TAG_GAME
    fieldtype_t::F_VECTOR,
    fieldtype_t::F_ANGLEHACK,
    fieldtype_t::F_EDICT,   // index on disk, pointer in memory
    fieldtype_t::F_ITEM,    // index on disk, pointer in memory
    fieldtype_t::F_CLIENT,  // index on disk, pointer in memory
    fieldtype_t::F_IGNORE
} fieldtype_t;
...
void T_Damage( edict* targ, edict* inflictor, edict* attacker, vec3_t dir,
               vec3_t point, vec3_t normal, int damage, int knockback,
               int dflags, int mod );
void T_RadiusDamage( edict* inflictor, edict* attacker, float damage,
                     edict* ignore, float radius, int mod );
...
#define DAMAGE_BULLET \
    0x00000010  // damage is from a bullet (used for ricochets)
...
void monster_fire_bfg( edict* self, vec3_t start, vec3_t aimdir, int damage,
                       int speed, int kick, float damage_radius,
                       int flashtype );
...
void fire_bullet( edict* self, vec3_t start, vec3_t aimdir, int damage,
                  int kick, int hspread, int vspread, int mod );
void fire_shotgun( edict* self, vec3_t start, vec3_t aimdir, int damage,
                   int kick, int hspread, int vspread, int count, int mod );
void fire_blaster( edict* self, vec3_t start, vec3_t aimdir, int damage,
                   int speed, int effect, bool hyper );
void fire_grenade( edict* self, vec3_t start, vec3_t aimdir, int damage,
                   int speed, float timer, float damage_radius );
void fire_grenade2( edict* self, vec3_t start, vec3_t aimdir, int damage,
                    int speed, float timer, float damage_radius, bool held );
void fire_rocket( edict* self, vec3_t start, vec3_t dir, int damage, int speed,
                  float damage_radius, int radius_damage );
void fire_rail( edict* self, vec3_t start, vec3_t aimdir, int damage,
                int kick );
void fire_bfg( edict* self, vec3_t start, vec3_t dir, int damage, int speed,
               float damage_radius );
...
void Weapon_Generic( edict* ent, int FRAME_ACTIVATE_LAST, int FRAME_FIRE_LAST,
