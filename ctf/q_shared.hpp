#ifndef ____CTF_Q_SHARED_H__
#define ____CTF_Q_SHARED_H__
...
typedef enum
{
    multicast_t::MULTICAST_ALL,
    multicast_t::MULTICAST_PHS,
    multicast_t::MULTICAST_PVS,
    multicast_t::MULTICAST_ALL_R,
    multicast_t::MULTICAST_PHS_R,
    multicast_t::MULTICAST_PVS_R
} multicast_t;
...
int BoxOnPlaneSide( vec3_t emins, vec3_t emaxs, struct plane_s* plane );
...
// void Com_sprintf(unsigned char* dest, int size, unsigned char *fmt, ...);
...
void Info_SetValueForKey( const std::string& s, const std::string& key,
...
#define MASK_MONSTERSOLID                                       \
    ( CONTENTS_SOLID | CONTENTS_MONSTERCLIP | CONTENTS_WINDOW | \
      CONTENTS_MONSTER )
...
// quake2::getInstance()->gi.BoxEdicts() can return a list of either solid or trigger entities
...
#define PMF_TIME_TELEPORT \
    32  // this structure needs to be communicated bit-accurate
...
#define PMF_NO_PREDICTION \
    64  // temporarily disables prediction (used for grappling hook)
...
#define EF_ANIM01 \
    0x00000400  // automatically cycle between frames 0 and 1 at 2 hz
#define EF_ANIM23 \
    0x00000800  // automatically cycle between frames 2 and 3 at 2 hz
#define EF_ANIM_ALL 0x00001000  // automatically cycle through all frames at 2hz
#define EF_ANIM_ALLFAST \
    0x00002000  // automatically cycle through all frames at 10hz
...
#define CHAN_NO_PHS_ADD \
    8  // send to all clients, not just ones in PHS (ATTN 0 will also do this)
...
typedef struct entity_state
{
    ...
    int solid;  // for client side prediction, 8*(bits 0-4) is x/y radius
                ...
                // quake2::getInstance()->gi.linkentity sets this properly
