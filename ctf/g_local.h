...
typedef struct
{
    ...
    qboolean    (*checkattack)(struct edict_s *self);
...
qboolean M_CheckAttack (struct edict_s *self);
...
qboolean infront (struct edict_s *self, struct edict_s *other);
qboolean visible (struct edict_s *self, struct edict_s *other);
qboolean FacingIdeal(struct edict_s *self);

//
// g_weapon.c
//
void ThrowDebris (struct edict_s *self, char *modelname, float speed, vec3_t origin);
qboolean fire_hit (struct edict_s *self, vec3_t aim, int damage, int kick);
...
void fire_blaster (struct edict_s *self, vec3_t start, vec3_t aimdir, int damage, int speed, int effect, qboolean hyper);
void fire_grenade (struct edict_s *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius);
void fire_grenade2 (struct edict_s *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius, qboolean held);
...
// m_move.c
//
qboolean M_CheckBottom (struct edict_s *ent);
qboolean M_walkmove (struct edict_s *ent, float yaw, float dist);
