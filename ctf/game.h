...
// ZOID
#define SVF_PROJECTILE                                                         \
  0x00000008 // entity is simple projectile, used for network optimization
...
// a constant velocity that don't change direction or model ZOID
...
typedef struct {
  ...
  void (*unlinkentity)(
      edict_t *ent); // call before removing an interactive edict
...
typedef struct {
  ...
  // the server console. The game can issue gi.argc() / gi.argv() commands to
