...
void MSG_WriteDeltaEntity( struct entity_state* from, struct entity_state* to,
...
int COM_Argc( void );
std::string COM_Argv( int arg );  // range and null checked
...
#define UPDATE_BACKUP \
    16  // copies of entity_state to keep buffered
...
cvar* Cvar_Get( char* var_name, char* value, int flags );
...
cvar* Cvar_FullSet( char* var_name, char* value, int flags );
...
void Netchan_OutOfBand( netsrc_t net_socket, netadr_t adr, int length,
                        byte* data );
